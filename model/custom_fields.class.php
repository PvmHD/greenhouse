...
class CustomFields {
     ...
     */
    public static function saveCustomDropdown($number, $dropdown_fields='') {
        
        if (!empty($dropdown_fields)) {

            global $obj_db;
            
            // example: 1|hhh|#3366cc, 2|nnnn|#3366cc,
            
            $arr_custom_fields = explode(',', $dropdown_fields);
                            ...
                            if (!empty($option)) {
                                if (!in_array($option_id, $arr_done_items)) {
                                    ...
                                    $str_query3 = 'INSERT INTO `custom_fields_options` (`custom_field_id`,`value`, `text`,`color`) ' .
                                        'VALUES ("'.$number.'", "' . $option_val . '", "' . $option . '", "' . $color . '") ';
     ...
     * @param type $value
     */
    public static function saveDropdownLabel($number, $value) {
        global $obj_db;
        
        $str_label = self::getDropdownLabel($number);
        if(!is_null($str_label)) {
            $str_query = 'UPDATE custom_fields SET `label` = "' . $value . '"' .
                        ' WHERE `field_id` = ' . $number;
            ...
            $str_query = 'INSERT INTO custom_fields SET `label` = "' . $value . '"' .
                        ' , `field_type` = "dropdown",  `field_id` = ' . $number;
    ...
    public static function getDropdownLabel($number) {
        global $obj_db;
    ...
    public static function getCustomDropdown($number, $bln_index=false) {
        global $obj_db;
        $arr_options = array();
        
        $str_query = 'SELECT * FROM custom_fields_options  ' .
                ' WHERE custom_field_id = ' . $number;
    ...
    // TODO when dropdown per calendar also check which calendar_id    
        $str_query = 'SELECT o.*, d.* FROM custom_fields_options o ' .
                ' LEFT JOIN `custom_fields` d ON(d.field_id = o.custom_field_id) WHERE option_id = ' . $option_id;
