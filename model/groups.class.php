...
class Group {
    ...
    public static function getGroups($exclude_admins=false) {
        global $obj_db;
        ...
        if (!empty($arr_groups)) {
            foreach ($arr_groups as &$group) {
                $str_query2 = 'SELECT * FROM group_users gu LEFT JOIN `users` u ON(u.user_id = gu.user_id) WHERE u.deleted = 0 AND u.active = 1 AND gu.group_id = ' . $group['group_id'];
                
                if ($exclude_admins) {
                    $str_query2 .= ' AND `usertype` = "user"';
                }
    ...
    public static function saveGroup($frm_submitted) {
        ...
        if (User::isLoggedIn()) {
            ...
            if (!empty($frm_submitted['groupusers'])) {
                // save groupusers
                $arr_user_ids = $frm_submitted['groupusers'];       //explode(',', $frm_submitted['groupusers']);
    ...
    public static function getGroupUsers($int_group_id = -1, $bln_simple = false) {
        ...
        $str_query = 'SELECT gu.*, CONCAT_WS(" ", u.firstname,u.infix,u.lastname) as fullname FROM group_users gu LEFT JOIN `users` u ON(gu.user_id = u.user_id) WHERE u.deleted = 0 AND u.`active` = 1 AND u.usertype = "user" AND `group_id` = ' . $int_group_id;

        $str_query .= ' ORDER BY `fullname`';
