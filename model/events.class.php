...
class Events {
    ...
    public static function getCountAssignedUsers($date, $int_group_id=-1, $arr_group_users=array()) {
        global $obj_db;
        $arr_return = array();
        
        
        $str_query = 'SELECT * FROM events WHERE `team_member_id` != "" AND `team_member_id` IS NOT NULL AND "'.$date.'" >= `date_start` AND "'.$date.'" <= `date_end`';
      //`assigned_by` IS NOT NULL AND `assigned_by` > 0 AND 
     
        $res = mysqli_query($obj_db, $str_query);

        if ($res !== false && !empty($res)) {
            while ($arr_line = mysqli_fetch_array($res, MYSQLI_ASSOC)) {
                if(strstr($arr_line['team_member_id'], ',')) {
                    $arr_teammembers = explode(',',$arr_line['team_member_id']);
                } else {
                    $arr_teammembers = array($arr_line['team_member_id']);
                }
                foreach($arr_teammembers as $teammember) {
                    if(!empty($arr_group_users)) {
                        if($int_group_id > 0) {
                            $teammember = str_replace($int_group_id.'_', '', $teammember);
                    
                            if(in_array($teammember, $arr_group_users)) {
                                if(!in_array($teammember, $arr_return)) {
                                    $arr_return[] = $teammember;
                                }
                            }
                        }
                    }
    ...
    public static function getLastAddedEvents($amount = 5, $ajax = false) {
        ...
        if (User::isLoggedIn()) {
            ...
            $str_query = 'SELECT e.*,c.*,l.*, concat_ws(" ",u.firstname,u.lastname) as fullname, concat_ws(" ",e.date_start,e.time_start) as start,concat_ws(" ",e.date_end,e.time_end) as end FROM `log` l';
            $str_query .= '  LEFT JOIN `events` e ON(e.event_id = l.event_id)';
            $str_query .= '  LEFT JOIN `users` u ON(u.user_id = l.user_id)';
            $str_query .= '  LEFT JOIN `calendars` c ON(c.calendar_id = e.calendar_id)';
            $str_query .= ' WHERE l.`info` = "insert"';
          ...
          //  $str_query = 'SELECT e.*,c.*, concat_ws(" ",e.date_start,e.time_start) as start,concat_ws(" ",e.date_end,e.time_end) as end FROM events e LEFT JOIN calendars c ON(c.calendar_id = e.calendar_id) ';
            ...
            $str_query .= ' ORDER BY l.`date` DESC LIMIT ' . $amount;
    // echo $str_query;       
...
//            echo '<pre>'.print_r((array)$response, true).'</pre>';
    ...
    public static function getEvents($frm_submitted) {
        global $obj_db;
        $arr_content = array();
        $arr_loggedin_user = array();

        $bln_loggedin = User::isLoggedIn();
        
        if ($bln_loggedin) {
            $arr_loggedin_user = User::getUser();
            $bln_is_user = User::isUser();
        }
        ...
        $str_query = 'SELECT *, event_id as id, concat_ws(" ",date_start,time_start) as start,concat_ws(" ",date_end,time_end) as end FROM events e ' .
                ...
                ' WHERE c.calendar_id IS NOT NULL AND c.deleted = 0 AND ((date_start BETWEEN "' . $str_start . '" AND "' . $str_end . '") OR ( date_end BETWEEN "' . $str_start . '" AND "' . $str_end . '")' .
        ...
        if (!empty($frm_submitted['uid'])) {
            if(!strstr($frm_submitted['uid'], ',')) {
                $str_query .= ' AND (e.user_id = ' . $frm_submitted['uid'] . ' OR FIND_IN_SET('. $frm_submitted['uid'] . ',`team_member_id`))';
            } else {
               // $str_query .= ' AND e.user_id = ' . $frm_submitted['uid'];
            }
        }
       ...
       // $arr_drd2_fields = CustomFields::getCustomDropdown2(true);
        ...
        if(!empty($frm_submitted['option_id'])) {    
            if($bln_linked_dropdowns == 'on') {
                $str_query .= ' AND (((e.dropdown1_option_id IS NULL OR e.dropdown1_option_id = -1) AND (e.dropdown2_option_id IS NULL OR e.dropdown2_option_id = -1)) OR ((e.dropdown1_option_id IN (' . $frm_submitted['option_id'] . ')) AND (e.dropdown2_option_id IN (' . $frm_submitted['option_id'] . '))))';
            } else {
                $str_query .= ' AND (((e.dropdown1_option_id IS NULL OR e.dropdown1_option_id = -1) AND (e.dropdown2_option_id IS NULL OR e.dropdown2_option_id = -1)) OR ((e.dropdown1_option_id IN (' . $frm_submitted['option_id'] . ')) OR (e.dropdown2_option_id IN (' . $frm_submitted['option_id'] . '))))';
            }
        }
        
            
        
        $str_query .= ' order by e.`calendar_id`, `start`';
        ...
        while ($arr_line = mysqli_fetch_array($obj_result, MYSQLI_ASSOC)) {
            ...
            if(!empty($frm_submitted['uid']) && strstr($frm_submitted['uid'], ',')) {
                $arr_requested_users = explode(',', $frm_submitted['uid']);
                
                $found = false;
                if(in_array($arr_line['user_id'], $arr_requested_users)) {
                    $found = true;
                }
                
                $arr_event_teammembers = explode(',', $arr_line['team_member_id']);
                
                
                foreach($arr_requested_users as $req_user) {
                    if(in_array($req_user, $arr_event_teammembers)) {
                        $found = true;
                    }
                }
            ...
            if (isset($arr_calendar_options[$arr_line['calendar_id']]) && isset($arr_calendar_options[$arr_line['calendar_id']]['can_view_own_assigned']) && (bool)$arr_calendar_options[$arr_line['calendar_id']]['can_view_own_assigned']) {
                if (!$bln_loggedin || ($bln_loggedin && $bln_is_user && !is_null($arr_line['assigned_by']) && $arr_line['assigned_by'] > 0 && $arr_loggedin_user['user_id'] !== $arr_line['user_id'])) {
                    continue;
            ...
            // alterable period
            if(User::isUser()) {
                if(isset($arr_cal['mysql_alterable_startdate']) && !empty($arr_cal['mysql_alterable_startdate']) && isset($arr_cal['mysql_alterable_enddate']) && !empty($arr_cal['mysql_alterable_enddate'])) {
                    if((strtotime($arr_line['date_start']) >= strtotime($arr_cal['mysql_alterable_startdate']) && strtotime($arr_line['date_start']) <= strtotime($arr_cal['mysql_alterable_enddate'])) ||
                            (strtotime($arr_line['date_end']) >= strtotime($arr_cal['mysql_alterable_startdate']) && strtotime($arr_line['date_end']) <= strtotime($arr_cal['mysql_alterable_enddate']))) {
                        // do nothing
                    } else {
                        $arr_line['allowEdit'] = false;
                        $arr_line['editable'] = false;
                        $arr_line['deletable'] = false;
                    }
                }
            }
            ...
            $arr_line['calendar_name'] = $arr_cal['name'];
      
            $arr_line['show_description_field'] = isset($arr_calendar_options[$arr_line['calendar_id']]['show_description_field']) ? (bool) $arr_calendar_options[$arr_line['calendar_id']]['show_description_field'] : true; 
            $arr_line['show_location_field'] = isset($arr_calendar_options[$arr_line['calendar_id']]['show_location_field']) ? (bool) $arr_calendar_options[$arr_line['calendar_id']]['show_location_field'] : false; 
            $arr_line['show_phone_field'] = isset($arr_calendar_options[$arr_line['calendar_id']]['show_phone_field']) ? (bool) $arr_calendar_options[$arr_line['calendar_id']]['show_phone_field'] : false; 
            $arr_line['show_team_member_field'] = isset($arr_calendar_options[$arr_line['calendar_id']]['show_team_member_field']) ? (bool) $arr_calendar_options[$arr_line['calendar_id']]['show_team_member_field'] : false; 
            $arr_line['allday_is_default'] = isset($arr_calendar_options[$arr_line['calendar_id']]['allday_is_default']) ? (bool) $arr_calendar_options[$arr_line['calendar_id']]['allday_is_default'] : false; 
            $arr_line['show_url_field'] = isset($arr_calendar_options[$arr_line['calendar_id']]['show_url_field']) ? (bool) $arr_calendar_options[$arr_line['calendar_id']]['show_url_field'] : false; 
            $arr_line['show_dropdown_1_field'] = isset($arr_calendar_options[$arr_line['calendar_id']]['show_dropdown_1_field']) ? (bool) $arr_calendar_options[$arr_line['calendar_id']]['show_dropdown_1_field'] : false; 
            $arr_line['show_dropdown_2_field'] = isset($arr_calendar_options[$arr_line['calendar_id']]['show_dropdown_2_field']) ? (bool) $arr_calendar_options[$arr_line['calendar_id']]['show_dropdown_2_field'] : false; 
            $arr_line['show_dropdown_3_field'] = isset($arr_calendar_options[$arr_line['calendar_id']]['show_dropdown_3_field']) ? (bool) $arr_calendar_options[$arr_line['calendar_id']]['show_dropdown_3_field'] : false; 
            $arr_line['show_dropdown_4_field'] = isset($arr_calendar_options[$arr_line['calendar_id']]['show_dropdown_4_field']) ? (bool) $arr_calendar_options[$arr_line['calendar_id']]['show_dropdown_4_field'] : false; 
            $arr_line['show_dropdown_5_field'] = isset($arr_calendar_options[$arr_line['calendar_id']]['show_dropdown_5_field']) ? (bool) $arr_calendar_options[$arr_line['calendar_id']]['show_dropdown_5_field'] : false; 
            
            $arr_line['add_team_member_to_title'] = isset($arr_calendar_options[$arr_line['calendar_id']]['add_team_member_to_title']) ? (bool) $arr_calendar_options[$arr_line['calendar_id']]['add_team_member_to_title'] : true; 
            $arr_line['add_custom_dropdown1_to_title'] = isset($arr_calendar_options[$arr_line['calendar_id']]['add_custom_dropdown1_to_title']) ? (bool) $arr_calendar_options[$arr_line['calendar_id']]['add_custom_dropdown1_to_title'] : true; 
            $arr_line['add_custom_dropdown2_to_title'] = isset($arr_calendar_options[$arr_line['calendar_id']]['add_custom_dropdown2_to_title']) ? (bool) $arr_calendar_options[$arr_line['calendar_id']]['add_custom_dropdown2_to_title'] : true; 
            $arr_line['add_custom_dropdown3_to_title'] = isset($arr_calendar_options[$arr_line['calendar_id']]['add_custom_dropdown3_to_title']) ? (bool) $arr_calendar_options[$arr_line['calendar_id']]['add_custom_dropdown3_to_title'] : true; 
            $arr_line['add_custom_dropdown4_to_title'] = isset($arr_calendar_options[$arr_line['calendar_id']]['add_custom_dropdown4_to_title']) ? (bool) $arr_calendar_options[$arr_line['calendar_id']]['add_custom_dropdown4_to_title'] : true; 
            $arr_line['add_custom_dropdown5_to_title'] = isset($arr_calendar_options[$arr_line['calendar_id']]['add_custom_dropdown5_to_title']) ? (bool) $arr_calendar_options[$arr_line['calendar_id']]['add_custom_dropdown5_to_title'] : true; 
            $arr_line['calendarname_is_title'] = isset($arr_calendar_options[$arr_line['calendar_id']]['calendarname_is_title']) ? (bool) $arr_calendar_options[$arr_line['calendar_id']]['calendarname_is_title'] : false; 
            ...
            $arr_line['textColor'] = 'white';
            if(isset($arr_cal['calendar_textcolor']) && !empty($arr_cal['calendar_textcolor'])) {
                $arr_line['textColor'] = $arr_cal['calendar_textcolor'];
            } else if (defined('EVENT_TEXT_COLOR')) {
                $event_text_color = EVENT_TEXT_COLOR;
                if (!empty($event_text_color)) {
                    $arr_line['textColor'] = $event_text_color;
                }
            }
            ...
            if ($arr_line['repeating_event_id'] > 0) {
                ...
                if(isset($arr_cal['calendar_textcolor']) && !empty($arr_cal['calendar_textcolor'])) {
                    $arr_line['textColor'] = $arr_cal['calendar_textcolor'];
                } else if (defined('RECURRING_EVENT_TEXT_COLOR')) {
            ...
            if (isset($_SESSION['employee-work-schedule-sd']) && isset($_SESSION['employee-work-schedule-ft'])) {
                if (substr($arr_line['start'], 0, 10) == $_SESSION['employee-work-schedule-sd'] && $arr_line['event_id'] == $_SESSION['employee-work-schedule-ft']) {   //$arr_line['title'] == $_SESSION['employee-work-schedule-ft']) {
            ...
            if (isset($arr_line['team_member_id']) && !empty($arr_line['team_member_id']) && $arr_line['team_member_id'] != -1) {
                $arr_line['team_member'] = Utils::getTeamMemberNames($arr_line['team_member_id']);
            ...
            if($arr_line['show_dropdown_1_field']) {
                if(isset($arr_line['dropdown1_option_id']) && $arr_line['dropdown1_option_id'] > 0) {
                    ...
                    $arr_line['dropdown1_label'] = $arr_option['label'];
            ...
            if($arr_line['show_dropdown_2_field']) {
                if(isset($arr_line['dropdown2_option_id']) && $arr_line['dropdown2_option_id'] > 0) {
                    ...
                    $arr_line['dropdown2_label'] = $arr_option['label'];
            ...
            $arr_line['dropdown3'] = '';
            if($arr_line['show_dropdown_3_field']) {
                if(isset($arr_line['dropdown3_option_id']) && $arr_line['dropdown3_option_id'] > 0) {
                    if (isset($_SESSION['ews_custom_field_options']) && isset($_SESSION['ews_custom_field_options'][$arr_line['dropdown3_option_id']]) && !empty($_SESSION['ews_custom_field_options'][$arr_line['dropdown3_option_id']])) {
                        $arr_option = $_SESSION['ews_custom_field_options'][$arr_line['dropdown3_option_id']];
                    } else {
                        $arr_option = CustomFields::getCustomDropdownOption($arr_line['dropdown3_option_id']);
                        $_SESSION['ews_custom_field_options'][$arr_line['dropdown3_option_id']] = $arr_option;
                    }
                    $arr_line['dropdown3_label'] = $arr_option['label'];
                    $arr_line['dropdown3'] = $arr_option['text'];
                    $arr_line['dropdown3_color'] = $arr_option['color'];
                }
            }
            
            $arr_line['dropdown4'] = '';
            if($arr_line['show_dropdown_4_field']) {
                if(isset($arr_line['dropdown4_option_id']) && $arr_line['dropdown4_option_id'] > 0) {
                    if (isset($_SESSION['ews_custom_field_options']) && isset($_SESSION['ews_custom_field_options'][$arr_line['dropdown4_option_id']]) && !empty($_SESSION['ews_custom_field_options'][$arr_line['dropdown4_option_id']])) {
                        $arr_option = $_SESSION['ews_custom_field_options'][$arr_line['dropdown4_option_id']];
                    } else {
                        $arr_option = CustomFields::getCustomDropdownOption($arr_line['dropdown4_option_id']);
                        $_SESSION['ews_custom_field_options'][$arr_line['dropdown4_option_id']] = $arr_option;
                    }
                    $arr_line['dropdown4_label'] = $arr_option['label'];
                    $arr_line['dropdown4'] = $arr_option['text'];
                    $arr_line['dropdown4_color'] = $arr_option['color'];
                }
            }
            
            $arr_line['dropdown5'] = '';
            if($arr_line['show_dropdown_5_field']) {
                if(isset($arr_line['dropdown5_option_id']) && $arr_line['dropdown5_option_id'] > 0) {
                    if (isset($_SESSION['ews_custom_field_options']) && isset($_SESSION['ews_custom_field_options'][$arr_line['dropdown5_option_id']]) && !empty($_SESSION['ews_custom_field_options'][$arr_line['dropdown5_option_id']])) {
                        $arr_option = $_SESSION['ews_custom_field_options'][$arr_line['dropdown5_option_id']];
                    } else {
                        $arr_option = CustomFields::getCustomDropdownOption($arr_line['dropdown5_option_id']);
                        $_SESSION['ews_custom_field_options'][$arr_line['dropdown5_option_id']] = $arr_option;
                    }
                    $arr_line['dropdown5_label'] = $arr_option['label'];
                    $arr_line['dropdown5'] = $arr_option['text'];
                    $arr_line['dropdown5_color'] = $arr_option['color'];
                }
            }
            ...
            // is this a private group calendar: which users are in the usergroup
        
            if($arr_cal['share_type'] == 'private_group') {
                $arr_line['users'] = $arr_cal['users'];
            }
    ...
    public static function insertEvent($frm_submitted, $current_user_id = '', $from_usergroup_dditem=false) {
        ...
        if(!isset($frm_submitted['color']) || stristr($frm_submitted['color'], 'hsv')) {
            $frm_submitted['color'] = $arr_calendar['calendar_color'];
        }
        if(!isset($frm_submitted['textcolor']) || stristr($frm_submitted['textcolor'], 'hsv')) {
            $frm_submitted['textcolor'] = $arr_calendar['calendar_textcolor'];
        }
        ...
        } else if ($bln_isloggedin && !empty($frm_submitted['team_member_id']) && !strstr($frm_submitted['team_member_id'], ',') && (bool)$frm_submitted['assign'] && !empty($arr_calendar) && isset($arr_calendar['show_team_member_field']) && (bool)$arr_calendar['show_team_member_field']) {
            $arr_groupid_userid = explode('_', $frm_submitted['team_member_id']);
            
            $current_user_id = $arr_groupid_userid[1];
            $assigned_by = $arr_user['user_id'];
        } else {
        ...
        if ($bln_isloggedin || (!$bln_isloggedin && $arr_calendar['share_type'] == 'public' && $arr_calendar['can_add']) || ($is_repeating_event && ($arr_calendar['can_edit'] || $arr_calendar['can_add']))) {
            ...
            $str_query = 'INSERT INTO events ( title, description, calendar_id, location, phone, myurl, repeating_event_id, user_id, assigned_by, color, team_member_id, dropdown1_option_id, dropdown2_option_id, dropdown3_option_id, dropdown4_option_id, dropdown5_option_id, date_start, time_start, date_end, time_end, create_date, allday) ' .
                    ...
                    '"' . $frm_submitted['dropdown3_option_id'] . '",' .
                    '"' . $frm_submitted['dropdown4_option_id'] . '",' .
                    '"' . $frm_submitted['dropdown5_option_id'] . '",' .
                    '"' . $str_startdate . '",' .
                    '"' . $str_starttime . '",' .
                    '"' . $str_enddate . '",' .
                    '"' . $str_endtime . '",' .
            ...
            if ($obj_result !== false) {
                ...
                $arr_event['allday_is_default'] = (bool) CalendarOption::getOption('allday_is_default', $arr_event['calendar_id'], false);
                $arr_event['can_view_own_assigned'] = (bool) CalendarOption::getOption('can_view_own_assigned', $arr_event['calendar_id'], true);
                ...
                $arr_event['show_dropdown_3_field'] = (bool) CalendarOption::getOption('show_dropdown_3_field', $arr_event['calendar_id'], false);
                $arr_event['show_dropdown_4_field'] = (bool) CalendarOption::getOption('show_dropdown_4_field', $arr_event['calendar_id'], false);
                $arr_event['show_dropdown_5_field'] = (bool) CalendarOption::getOption('show_dropdown_5_field', $arr_event['calendar_id'], false);
            ...
            if (isset($arr_event['team_member_id']) && !empty($arr_event['team_member_id']) &&  $arr_event['team_member_id'] != -1) {
                $arr_event['team_member'] = Utils::getTeamMemberNames($arr_event['team_member_id']);
            ...
            $arr_event['dropdown3'] = '';
            if($arr_event['show_dropdown_3_field']) {
                if(isset($arr_event['dropdown3_option_id']) && $arr_event['dropdown3_option_id'] > 0) {
                    if (isset($_SESSION['ews_custom_field_options']) && isset($_SESSION['ews_custom_field_options'][$arr_event['dropdown3_option_id']]) && !empty($_SESSION['ews_custom_field_options'][$arr_event['dropdown3_option_id']])) {
                        $arr_option = $_SESSION['ews_custom_field_options'][$arr_event['dropdown3_option_id']];
                    } else {
                        $arr_option = CustomFields::getCustomDropdownOption($arr_event['dropdown3_option_id']);
                        $_SESSION['ews_custom_field_options'][$arr_event['dropdown3_option_id']] = $arr_option;
                    }
                    $arr_event['dropdown3'] = $arr_option['text'];
                }
            }
            $arr_event['dropdown4'] = '';
            if($arr_event['show_dropdown_4_field']) {
                if(isset($arr_event['dropdown4_option_id']) && $arr_event['dropdown4_option_id'] > 0) {
                    if (isset($_SESSION['ews_custom_field_options']) && isset($_SESSION['ews_custom_field_options'][$arr_event['dropdown4_option_id']]) && !empty($_SESSION['ews_custom_field_options'][$arr_event['dropdown4_option_id']])) {
                        $arr_option = $_SESSION['ews_custom_field_options'][$arr_event['dropdown4_option_id']];
                    } else {
                        $arr_option = CustomFields::getCustomDropdownOption($arr_event['dropdown4_option_id']);
                        $_SESSION['ews_custom_field_options'][$arr_event['dropdown4_option_id']] = $arr_option;
                    }
                    $arr_event['dropdown4'] = $arr_option['text'];
                }
            }
            $arr_event['dropdown5'] = '';
            if($arr_event['show_dropdown_5_field']) {
                if(isset($arr_event['dropdown5_option_id']) && $arr_event['dropdown5_option_id'] > 0) {
                    if (isset($_SESSION['ews_custom_field_options']) && isset($_SESSION['ews_custom_field_options'][$arr_event['dropdown5_option_id']]) && !empty($_SESSION['ews_custom_field_options'][$arr_event['dropdown5_option_id']])) {
                        $arr_option = $_SESSION['ews_custom_field_options'][$arr_event['dropdown5_option_id']];
                    } else {
                        $arr_option = CustomFields::getCustomDropdownOption($arr_event['dropdown5_option_id']);
                        $_SESSION['ews_custom_field_options'][$arr_event['dropdown5_option_id']] = $arr_option;
                    }
                    $arr_event['dropdown5'] = $arr_option['text'];
                }
            }
            
            $arr_event['textColor'] = 'white';
            if(isset($arr_calendar['calendar_textcolor']) && !empty($arr_calendar['calendar_textcolor'])) {
                $arr_event['textColor'] = $arr_calendar['calendar_textcolor'];
            } else if (defined('EVENT_TEXT_COLOR')) {
                $event_text_color = EVENT_TEXT_COLOR;
                if (!empty($event_text_color)) {
                    $arr_event['textColor'] = $event_text_color;
                }
            }
    ...
    public static function customviewUpdateEvent($frm_submitted) {
        global $obj_db;
        $arr_event = array();
        ...
        $str_query = 'UPDATE events SET `change_date` = "' . date('Y-m-d H:i:s') . '" ' .
                ...
                (!empty($frm_submitted['team_member_id']) && $frm_submitted['team_member_id'] == -1 || $frm_submitted['team_member_id'] > 0 ? ', `team_member_id` = "' . $frm_submitted['team_member_id'] . '"' : '') .
        ...
        $str_query .= ' WHERE `event_id` = ' . $frm_submitted['event_id'];

        $obj_result = mysqli_query($obj_db, $str_query);
        
    }
    
    public static function updateTitle($str_title='', $int_event_id=-1) {
        global $obj_db;
        $arr_event = array();
        
        if(!empty($str_title) && $int_event_id > 0) {
            $str_query = 'UPDATE events SET `title` = "' . $str_title . '" WHERE `event_id` = '. $int_event_id;
         
            $obj_result = mysqli_query($obj_db, $str_query);

            if ($obj_result !== false) {
                $str_query2 = 'SELECT *, event_id as id, concat_ws(" ",date_start,time_start) as start,concat_ws(" ",date_end,time_end) as end ' .
                        'FROM events WHERE event_id = ' . $int_event_id;
...
//        }
        ...
        $current_user_id = '';
        $assigned_by = '';
        if (!empty($frm_submitted['team_member_id']) && !strstr($frm_submitted['team_member_id'], ',')) {
            // teammember selected
        ...
        if($frm_submitted['intern_dd']) {
            ...
            $str_query = 'UPDATE events SET `change_date` = "' . date('Y-m-d H:i:s') . '" ' .
                ...
                (!empty($frm_submitted['dropdown3_option_id']) && $frm_submitted['dropdown3_option_id'] > 0 ? ', `dropdown3_option_id` = "' . $frm_submitted['dropdown3_option_id'] . '"' : '') .
                (!empty($frm_submitted['dropdown4_option_id']) && $frm_submitted['dropdown4_option_id'] > 0 ? ', `dropdown4_option_id` = "' . $frm_submitted['dropdown4_option_id'] . '"' : '') .
                (!empty($frm_submitted['dropdown5_option_id']) && $frm_submitted['dropdown5_option_id'] > 0 ? ', `dropdown5_option_id` = "' . $frm_submitted['dropdown5_option_id'] . '"' : '') .
...
//echo $str_query;
        ...
        if ($obj_result !== false) {
            ...
            $event['show_dropdown_1_field'] = CalendarOption::getOption('show_dropdown_1_field', $arr_event['calendar_id'], false);
            $event['show_dropdown_2_field'] = CalendarOption::getOption('show_dropdown_2_field', $arr_event['calendar_id'], false);
            $event['show_dropdown_3_field'] = CalendarOption::getOption('show_dropdown_3_field', $arr_event['calendar_id'], false);
            $event['show_dropdown_4_field'] = CalendarOption::getOption('show_dropdown_4_field', $arr_event['calendar_id'], false);
            $event['show_dropdown_5_field'] = CalendarOption::getOption('show_dropdown_5_field', $arr_event['calendar_id'], false);
      
            if($event['show_dropdown_1_field']) {
                if(isset($arr_event['dropdown1_option_id']) && $arr_event['dropdown1_option_id'] > 0) {
                    if (isset($_SESSION['ews_custom_field_options']) && isset($_SESSION['ews_custom_field_options'][$arr_event['dropdown1_option_id']]) && !empty($_SESSION['ews_custom_field_options'][$arr_event['dropdown1_option_id']])) {
                        $arr_option = $_SESSION['ews_custom_field_options'][$arr_event['dropdown1_option_id']];
                    } else {
            ...
            if($event['show_dropdown_2_field']) {
                if(isset($arr_event['dropdown2_option_id']) && $arr_event['dropdown2_option_id'] > 0) {
                    if (isset($_SESSION['ews_custom_field_options']) && isset($_SESSION['ews_custom_field_options'][$arr_event['dropdown2_option_id']]) && !empty($_SESSION['ews_custom_field_options'][$arr_event['dropdown2_option_id']])) {
                        $arr_option = $_SESSION['ews_custom_field_options'][$arr_event['dropdown2_option_id']];
                    } else {
            ...
            if($event['show_dropdown_3_field']) {
                if(isset($arr_event['dropdown3_option_id']) && $arr_event['dropdown3_option_id'] > 0) {
                    if (isset($_SESSION['ews_custom_field_options']) && isset($_SESSION['ews_custom_field_options'][$arr_event['dropdown3_option_id']]) && !empty($_SESSION['ews_custom_field_options'][$arr_event['dropdown3_option_id']])) {
                        $arr_option = $_SESSION['ews_custom_field_options'][$arr_event['dropdown3_option_id']];
                    } else {
                    ...
                    $arr_event['add_custom_dropdown3_to_title'] = isset($arr_calendar['add_custom_dropdown3_to_title']) ? (bool) $arr_calendar['add_custom_dropdown3_to_title'] : true; 
                    $arr_event['dropdown3'] = $arr_option['text'];
                    $arr_event['dropdown3_color'] = $arr_option['color'];
                }
            }
            
            if($event['show_dropdown_4_field']) {
                if(isset($arr_event['dropdown4_option_id']) && $arr_event['dropdown4_option_id'] > 0) {
                    if (isset($_SESSION['ews_custom_field_options']) && isset($_SESSION['ews_custom_field_options'][$arr_event['dropdown4_option_id']]) && !empty($_SESSION['ews_custom_field_options'][$arr_event['dropdown4_option_id']])) {
                        $arr_option = $_SESSION['ews_custom_field_options'][$arr_event['dropdown4_option_id']];
                    } else {
                    ...
                    $arr_event['add_custom_dropdown4_to_title'] = isset($arr_calendar['add_custom_dropdown4_to_title']) ? (bool) $arr_calendar['add_custom_dropdown4_to_title'] : true; 
                    $arr_event['dropdown4'] = $arr_option['text'];
                    $arr_event['dropdown4_color'] = $arr_option['color'];
                }
            }
            
            if($event['show_dropdown_5_field']) {
                if(isset($arr_event['dropdown5_option_id']) && $arr_event['dropdown5_option_id'] > 0) {
                    if (isset($_SESSION['ews_custom_field_options']) && isset($_SESSION['ews_custom_field_options'][$arr_event['dropdown5_option_id']]) && !empty($_SESSION['ews_custom_field_options'][$arr_event['dropdown5_option_id']])) {
                        $arr_option = $_SESSION['ews_custom_field_options'][$arr_event['dropdown5_option_id']];
                    } else {
                    ...
                    $arr_event['add_custom_dropdown5_to_title'] = isset($arr_calendar['add_custom_dropdown5_to_title']) ? (bool) $arr_calendar['add_custom_dropdown5_to_title'] : true; 
                    $arr_event['dropdown5'] = $arr_option['text'];
                    $arr_event['dropdown5_color'] = $arr_option['color'];
                }
            }
        ...
        if ($obj_result !== false) {
            ...
            if (isset($arr_event['team_member_id']) && $arr_event['team_member_id'] > 0) {
                if (isset($_SESSION['ews_users']) && isset($_SESSION['ews_users'][$arr_event['team_member_id']]) && !empty($_SESSION['ews_users'][$arr_event['team_member_id']])) {
                    ...
                    $_SESSION['ews_users'][$arr_event['team_member_id']] = $arr_team_member;
                }
...
//	}
    ...
    public static function getRepeatingevent($int_repeating_event=-1) {
        if($int_repeating_event > 0) {
            global $obj_db;
        
            $arr_content = array();

            $str_query = 'SELECT * FROM repeating_events WHERE rep_event_id = ' . $int_repeating_event;
...
//            // do nothing, $current_user_id is used to assign to the correct user
//            $assigned_by = $arr_user['user_id'];
//        } else if ($bln_isloggedin && !empty($frm_submitted['team_member_id']) && !strstr($frm_submitted['team_member_id'], ',') && (int)$frm_submitted['team_member_id'] > 0 && (bool)$frm_submitted['assign'] && !empty($arr_calendar) && isset($arr_calendar['show_team_member_field']) && (bool)$arr_calendar['show_team_member_field'] && isset($arr_calendar['notify_assign_teammember']) && (bool) $arr_calendar['notify_assign_teammember']) {
//            $arr_groupid_userid = explode('_', $frm_submitted['team_member_id']);
//            
//            $current_user_id = $arr_groupid_userid[1];
//            $assigned_by = $arr_user['user_id'];
...
//        }
        ...
        $str_query_r = 'INSERT INTO events ( title, location, description, phone, myurl, team_member_id, assigned_by, dropdown1_option_id, dropdown2_option_id, dropdown3_option_id, dropdown4_option_id, dropdown5_option_id, calendar_id, repeating_event_id, user_id, color, date_start, time_start, date_end, time_end, create_date, allday) VALUES ';
    ...
    public static function updateRepeatingEvent($arr_dates, $frm_submitted) {
        ...
        if(!strstr($frm_submitted['team_member_id'], ',')) {
            if(!empty($frm_submitted['team_member_id']) && strstr($frm_submitted['team_member_id'], '_')) {
                ...
                $arr_teammember = explode('_', $frm_submitted['team_member_id']);
                $str_update_events_query .= '`user_id` = "' . $arr_teammember[1]. '", ';
                $str_update_events_query .= '`assigned_by` = "' . $assigned_by. '", ';
            }
        }
        ...
        $str_update_events_query .= (!empty($frm_submitted['dropdown3_option_id']) && $frm_submitted['dropdown3_option_id'] > 0 ? '`dropdown3_option_id` = "' . $frm_submitted['dropdown3_option_id'] . '", ' : '') ;
        $str_update_events_query .= (!empty($frm_submitted['dropdown4_option_id']) && $frm_submitted['dropdown4_option_id'] > 0 ? '`dropdown4_option_id` = "' . $frm_submitted['dropdown4_option_id'] . '", ' : '') ;
        $str_update_events_query .= (!empty($frm_submitted['dropdown5_option_id']) && $frm_submitted['dropdown5_option_id'] > 0 ? '`dropdown5_option_id` = "' . $frm_submitted['dropdown5_option_id'] . '", ' : '') ;
...
//				'AND '.$frm_submitted['date_start'].' BETWEEN date_start AND date_end';
 ...
 // ook kijken naar welke kalenders de ingelogde gebruiker mag zien
        if(isset($frm_submitted['cid'])) {
            $str_cal_ids = $frm_submitted['cid'];
        } else {
            $arr_calendars = Calendar::getCalendars(false,false,true);
            $str_cal_ids = implode(',', $arr_calendars);
        }
        
        $arr_return = array();
        
        if(empty($str_cal_ids)) {
        ...
        $str_query = 'SELECT * ,event_id AS id, concat_ws( " ", date_start, time_start ) AS START , concat_ws( " ", date_end, time_end ) AS END ' .
                'FROM events e LEFT JOIN `calendars` c ON(e.calendar_id = c.calendar_id) WHERE c.calendar_id IS NOT NULL AND c.deleted = 0 ';

        $str_query .= ' AND e.calendar_id IN('.$str_cal_ids.')';
        ...
        if(isset($frm_submitted['sq']) && !empty($frm_submitted['sq'])) {
            $str_query .= ' AND (`title` LIKE  "%' . $frm_submitted['sq'] . '%" OR `description` LIKE  "%' . $frm_submitted['sq'] . '%" OR `location` LIKE  "%' . $frm_submitted['sq'] . '%")' ;
        ...
        if($frm_submitted['sort'] == 'cal') {
            $sort_by_cal_order = true;
        }
        ...
        while ($arr_line = mysqli_fetch_array($obj_result, MYSQLI_ASSOC)) {
            ...
            $arr_line['show_dropdown_4_field'] = isset($arr_calendar_options[$arr_line['calendar_id']]['show_dropdown_4_field']) ? (bool) $arr_calendar_options[$arr_line['calendar_id']]['show_dropdown_4_field'] : false; 
            $arr_line['show_dropdown_5_field'] = isset($arr_calendar_options[$arr_line['calendar_id']]['show_dropdown_5_field']) ? (bool) $arr_calendar_options[$arr_line['calendar_id']]['show_dropdown_5_field'] : false; 
            $arr_line['add_custom_dropdown1_to_title'] = isset($arr_calendar_options[$arr_line['calendar_id']]['add_custom_dropdown1_to_title']) ? (bool) $arr_calendar_options[$arr_line['calendar_id']]['add_custom_dropdown1_to_title'] : false; 
            $arr_line['add_custom_dropdown2_to_title'] = isset($arr_calendar_options[$arr_line['calendar_id']]['add_custom_dropdown2_to_title']) ? (bool) $arr_calendar_options[$arr_line['calendar_id']]['add_custom_dropdown2_to_title'] : false; 
            $arr_line['add_custom_dropdown3_to_title'] = isset($arr_calendar_options[$arr_line['calendar_id']]['add_custom_dropdown3_to_title']) ? (bool) $arr_calendar_options[$arr_line['calendar_id']]['add_custom_dropdown3_to_title'] : false; 
            $arr_line['add_custom_dropdown4_to_title'] = isset($arr_calendar_options[$arr_line['calendar_id']]['add_custom_dropdown4_to_title']) ? (bool) $arr_calendar_options[$arr_line['calendar_id']]['add_custom_dropdown4_to_title'] : false; 
            $arr_line['add_custom_dropdown5_to_title'] = isset($arr_calendar_options[$arr_line['calendar_id']]['add_custom_dropdown5_to_title']) ? (bool) $arr_calendar_options[$arr_line['calendar_id']]['add_custom_dropdown5_to_title'] : false; 
...
//                    $_SESSION['ews_users'][$arr_line['team_member_id']] = $arr_team_member;
//                }
...
//            }
            ...
            if($arr_line['show_dropdown_1_field'] && $arr_line['add_custom_dropdown1_to_title']) {
                if (isset($arr_line['dropdown1_option_id']) && $arr_line['dropdown1_option_id'] > 0) {
                    if (isset($_SESSION['ews_custom_field_options']) && isset($_SESSION['ews_custom_field_options'][$arr_line['dropdown1_option_id']]) && !empty($_SESSION['ews_custom_field_options'][$arr_line['dropdown1_option_id']])) {
                        $arr_option = $_SESSION['ews_custom_field_options'][$arr_line['dropdown1_option_id']];
                    } else {
                    ...
                    $arr_line['drd_1'] = $arr_option['text'];
                    $arr_line['drd_1_color'] = $arr_option['color'];
                }
            }
            
            if($arr_line['show_dropdown_2_field'] && $arr_line['add_custom_dropdown2_to_title']) {
                if (isset($arr_line['dropdown2_option_id']) && $arr_line['dropdown2_option_id'] > 0) {
                    if (isset($_SESSION['ews_custom_field_options']) && isset($_SESSION['ews_custom_field_options'][$arr_line['dropdown2_option_id']]) && !empty($_SESSION['ews_custom_field_options'][$arr_line['dropdown2_option_id']])) {
                        $arr_option = $_SESSION['ews_custom_field_options'][$arr_line['dropdown2_option_id']];
                    } else {
                    ...
                    $arr_line['drd_2'] = $arr_option['text'];
                    $arr_line['drd_2_color'] = $arr_option['color'];
                }
            }
            
            if($arr_line['show_dropdown_3_field'] && $arr_line['add_custom_dropdown3_to_title']) {
                if (isset($arr_line['dropdown3_option_id']) && $arr_line['dropdown3_option_id'] > 0) {
                    if (isset($_SESSION['ews_custom_field_options']) && isset($_SESSION['ews_custom_field_options'][$arr_line['dropdown3_option_id']]) && !empty($_SESSION['ews_custom_field_options'][$arr_line['dropdown3_option_id']])) {
                        $arr_option = $_SESSION['ews_custom_field_options'][$arr_line['dropdown3_option_id']];
                    } else {
                    ...
                    $arr_line['drd_3'] = $arr_option['text'];
                    $arr_line['drd_3_color'] = $arr_option['color'];
                }
            }
            
            if($arr_line['show_dropdown_4_field'] && $arr_line['add_custom_dropdown4_to_title']) {
                if (isset($arr_line['dropdown4_option_id']) && $arr_line['dropdown4_option_id'] > 0) {
                    if (isset($_SESSION['ews_custom_field_options']) && isset($_SESSION['ews_custom_field_options'][$arr_line['dropdown4_option_id']]) && !empty($_SESSION['ews_custom_field_options'][$arr_line['dropdown4_option_id']])) {
                        $arr_option = $_SESSION['ews_custom_field_options'][$arr_line['dropdown4_option_id']];
                    } else {
                    ...
                    $arr_line['drd_4'] = $arr_option['text'];
                    $arr_line['drd_4_color'] = $arr_option['color'];
                }
            }
            
            if($arr_line['show_dropdown_5_field'] && $arr_line['add_custom_dropdown5_to_title']) {
                if (isset($arr_line['dropdown5_option_id']) && $arr_line['dropdown5_option_id'] > 0) {
                    if (isset($_SESSION['ews_custom_field_options']) && isset($_SESSION['ews_custom_field_options'][$arr_line['dropdown5_option_id']]) && !empty($_SESSION['ews_custom_field_options'][$arr_line['dropdown5_option_id']])) {
                        $arr_option = $_SESSION['ews_custom_field_options'][$arr_line['dropdown5_option_id']];
                    } else {
                    ...
                    $arr_line['drd_5'] = $arr_option['text'];
                    $arr_line['drd_5_color'] = $arr_option['color'];
                }
            }
            
            if($sort_by_cal_order) {
                $arr_line['sorter'] = $arr_line['order_id'];
            }
    ...
    public static function getAgendaItems($arr_content, $frm_submitted = array(), $sort_by_cal_order = false, $lang='EN') {
        ...
        foreach ($arr_content as &$event) {
            ...
            if ($event['date_end'] != $event['date_start']) {
                ...
                if ((defined('COMBINE_MOREDAYS_EVENTS') && COMBINE_MOREDAYS_EVENTS)) {
                    if (defined('ENDDATE_OF_COMBINED_MOREDAYS_EVENTS_TEXT')) {
                        ...
                        if($lang == 'EN') {
                            $date = strftime('%B %e, %Y', strtotime($event['date_end']));
                        } else {
                            $date = strftime('%e %B %Y', strtotime($event['date_end']));
                        }
                        $event['title'] = '<b>-> ' . $date . '</b>: ' . $event['title'];
                    ...
                    foreach ($days_in_between as $k => $day) {
                        
                        if($k == 0) {
                            $event['point_end'] = true;
                        } else if($k == (count($days_in_between) - 1)) {
                            $event['point_start'] = true;
                            $event['point_end'] = false;
                        } else {
                            $event['point_start'] = true;
                            $event['point_end'] = true;
                        }
   ...
   //  setLocale(LC_ALL, array('Dutch_Netherlands.1252'));
