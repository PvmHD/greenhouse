...
class Calendar {
    ...
    public static function insertFirstCalendar() {
        ...
        $str_query = "INSERT INTO `calendars` (`name`, `share_type`, `calendar_color`, `calendar_textcolor`,`login_required`, `cal_startdate`, `cal_enddate`, `alterable_startdate`, `alterable_enddate`, `creator_id`, `can_view`, `can_add`, `can_edit`, `can_delete`, `can_change_color`, `can_dd_drag`, `initial_show`, `active`, `deleted`, `calendar_admin_email`, `users_can_email_event`, `all_event_mods_to_admin`, `all_event_mods_to_admin_when_loggedin`) VALUES " .
                "('Cal 1', 'public', '#FF5F3F', '#FFFFFF', 0, NULL, NULL, NULL, NULL, 2, 0, 1, 1, 1, 0, 'everyone', 0, 'yes', 0, NULL, 0, 0)";
...
//        }
    ...
    public static function calMailEventModsToAdmin($arr_cal) {
        $bln_continue = false;
        if(isset($arr_cal['all_event_mods_to_admin_when_loggedin']) && (bool) $arr_cal['all_event_mods_to_admin_when_loggedin']) {
            if (User::isLoggedIn()) {
                $bln_continue = true;
            }
        }
        if(isset($arr_cal['all_event_mods_to_admin_when_loggedin']) && (bool) $arr_cal['all_event_mods_to_admin_when_loggedin'] === false) {
            $bln_continue = true;
        }
    ...
    public static function getCalendar($cal_id, $dditems_as_string = false, $locations_as_string = false) {
        ...
        if (!isset($_SESSION['ews_calendars'])) {
            ...
            // new calendar options, so do a check
            if(!isset($_SESSION['ews_calendars'][$cal_id]['show_dropdown_3_field'])) {
        ...
        if ($obj_result1 !== false) {
            ...
            $arr_calendar['allday_is_default'] = (bool) CalendarOption::getOption('allday_is_default', $cal_id, false);
            $arr_calendar['can_view_own_assigned'] = (bool) CalendarOption::getOption('can_view_own_assigned', $cal_id, false);
            ...
            $arr_calendar['show_dropdown_3_field'] = (bool) CalendarOption::getOption('show_dropdown_3_field', $cal_id, false);
            $arr_calendar['show_dropdown_4_field'] = (bool) CalendarOption::getOption('show_dropdown_4_field', $cal_id, false);
            $arr_calendar['show_dropdown_5_field'] = (bool) CalendarOption::getOption('show_dropdown_5_field', $cal_id, false);
            ...
            $arr_calendar['add_custom_dropdown3_to_title'] = (bool) CalendarOption::getOption('add_custom_dropdown3_to_title', $cal_id, true);
            $arr_calendar['add_custom_dropdown4_to_title'] = (bool) CalendarOption::getOption('add_custom_dropdown4_to_title', $cal_id, true);
            $arr_calendar['add_custom_dropdown5_to_title'] = (bool) CalendarOption::getOption('add_custom_dropdown5_to_title', $cal_id, true);
            $arr_calendar['calendarname_is_title'] = (bool) CalendarOption::getOption('calendarname_is_title', $cal_id, false);
            ...
            if (!empty($arr_calendar['alterable_startdate']) && !empty($arr_calendar['alterable_enddate'])) {
                $arr_calendar['mysql_alterable_startdate'] = $arr_calendar['alterable_startdate'];
                $arr_calendar['mysql_alterable_enddate'] = $arr_calendar['alterable_enddate'];
            ...
            if(!isset($arr_calendar['calendar_textcolor']) || is_null($arr_calendar['calendar_textcolor']) || empty($arr_calendar['calendar_textcolor'])) {
                $arr_calendar['calendar_textcolor'] = '#FFFFFF';
            }
            
            // users when it's a private group calendar
            if($arr_calendar['share_type'] == 'private_group') {
                //usergroup_id       
                if(isset($_SESSION['ews_calendars'][$cal_id]['users'])) {
                    $arr_calendar['users'] = $_SESSION['ews_calendars'][$cal_id]['users'];
                } else {
                    $arr_calendar['users'] = Group::getGroupUsers($arr_calendar['usergroup_id'], true);
                    $_SESSION['ews_calendars'][$cal_id]['users'] = $arr_calendar['users'];
                }
                ...
                foreach ($arr_calendars as &$cal) {
                    ...
                    if (array_key_exists($cal['calendar_id'], $arr_dditems_usergroup)) {
                        ...
                        if ($int_dditems_usergroup_id > 0) {
                            ...
                            if(isset($arr_tmp_groups[$int_dditems_usergroup_id])) {
                                $arr_usergroup = $arr_tmp_groups[$int_dditems_usergroup_id];
                            } else {
                                $arr_usergroup = Group::getGroup($int_dditems_usergroup_id);
                                $arr_tmp_groups[$int_dditems_usergroup_id] = $arr_usergroup;
                            }
...
//                    }
    ...
    public static function saveCalendar($frm_submitted) {
        ...
        if (strstr($frm_submitted['calendar_color'], 'hsv')) {
            $frm_submitted['calendar_color'] = '#3366cc';
        }
        if (substr($frm_submitted['calendar_textcolor'], 0, 1) != '#') {
            $frm_submitted['calendar_textcolor'] = '#' . $frm_submitted['calendar_textcolor'];
        }
        ...
        if ($frm_submitted['calendar_id'] > 0) {
            $str_query = 'UPDATE calendars SET `name` = "' . $frm_submitted['name'] . '", ' .
                    ...
                    '`calendar_textcolor` = "' . $frm_submitted['calendar_textcolor'] . '", ' .
                    ...
                    '`all_event_mods_to_admin_when_loggedin` = ' . $frm_submitted['all_event_mods_to_admin_when_loggedin'] . ', ' .
        ...
        if ($frm_submitted['calendar_id'] > 0) {
            ...
            $arr_options = array('description_field_required' => $frm_submitted['description_field_required'],
                ...
                'allday_is_default' => $frm_submitted['allday_is_default'],
                'can_view_own_assigned' => $frm_submitted['can_view_own_assigned'],
                ...
                'show_dropdown_3_field' => $frm_submitted['show_dropdown_3_field'],
                'show_dropdown_4_field' => $frm_submitted['show_dropdown_4_field'],
                'show_dropdown_5_field' => $frm_submitted['show_dropdown_5_field'],
                'add_team_member_to_title' => $frm_submitted['add_team_member_to_title'],
                'calendarname_is_title' => $frm_submitted['calendarname_is_title'],
                ...
                'add_custom_dropdown3_to_title' => $frm_submitted['add_custom_dropdown3_to_title'],
                'add_custom_dropdown4_to_title' => $frm_submitted['add_custom_dropdown4_to_title'],
                'add_custom_dropdown5_to_title' => $frm_submitted['add_custom_dropdown5_to_title'],
            ...
            if ($frm_submitted['dditems_usergroup_id'] > 0) {
                ...
                CalendarOption::removeOption('dditems_usergroup_id', $frm_submitted['calendar_id']);
                CalendarOption::saveOption('assign_dditem_to_user', 0, '', $frm_submitted['calendar_id']);
            }
...
//                    $can_see_dditems    = true;
            ...
            if (ONLY_ADMIN_CAN_SEE_DRAG_DROP_ITEMS) {
                $can_see_dditems = false;
            } else if (isset($arr_cal['can_dd_drag']) && $arr_cal['can_dd_drag'] == 'everyone') {
                $can_see_dditems = true;
            } else {
                $can_see_dditems = false;
            }
            ...
            // if public
            if (isset($arr_cal['share_type']) && $arr_cal['share_type'] == 'public') {
            ...
            if (isset($arr_cal['share_type']) && $arr_cal['share_type'] == 'private_group' && $arr_cal['can_view'] == 1) {
                $can_view = true;
                $can_add = false;
                $can_edit = false;
                $can_delete = false;
