...
class Settings {
    ...
    public static function getSettings($user_id = '') {
        ...
        $arr_settings = array('language', 'other_language', 'timezone', 'show_am_pm', 'default_view', 'show_view_type',
            ...
            'show_public_and_private_separately','calendar_title','topbar_cal_logo_or_title'
...
//                        'lang_page','lang_of','date_header','starttime_header','endtime_header','title_header'
        ...
        foreach ($arr_settings as &$name) {
            if ($name == 'other_language' && isset($arr_result[$name]) && !empty($arr_result[$name])) {

                if (!file_exists(FULLCAL_DIR . '/script/lang' . strtoupper($arr_result[$name]) . '.js')) {
...
//   allready taken care of in getAdminSeting         
//              if (!array_key_exists($name, $arr_result) || empty($arr_result[$name])) {
//                // get settings from admin of this user
//                $str_admin_id = User::getAdminOfUser($user_id);
//                if((int)$str_admin_id > 0) {
...
//            }
        ...
        $arr_result['dropdown_1'] = CustomFields::getCustomDropdown(1);
        $arr_result['dropdown_2'] = CustomFields::getCustomDropdown(2);
        $arr_result['dropdown_3'] = CustomFields::getCustomDropdown(3);
        $arr_result['dropdown_4'] = CustomFields::getCustomDropdown(4);
        $arr_result['dropdown_5'] = CustomFields::getCustomDropdown(5);
        $arr_result['dropdown1_label'] = CustomFields::getDropdownLabel(1);
        $arr_result['dropdown2_label'] = CustomFields::getDropdownLabel(2);
        $arr_result['dropdown3_label'] = CustomFields::getDropdownLabel(3);
        $arr_result['dropdown4_label'] = CustomFields::getDropdownLabel(4);
        $arr_result['dropdown5_label'] = CustomFields::getDropdownLabel(5);
    ...
    public static function saveSettings($arr_settings, $section = '', $user_id = '') {
        ...
        foreach ($arr_settings as $key => $value) {
            if($key == 'dropdown_1_options') {
                ...
                CustomFields::saveCustomDropdown(1, $value);
            } else if($key == 'dropdown_2_options') {
                CustomFields::saveCustomDropdown(2, $value);
            } else if($key == 'dropdown_3_options') {
                CustomFields::saveCustomDropdown(3, $value);
            } else if($key == 'dropdown_4_options') {
                CustomFields::saveCustomDropdown(4, $value);
            } else if($key == 'dropdown_5_options') {
                CustomFields::saveCustomDropdown(5, $value);
            
                
            } else if($key == 'dropdown1_label') {
                // save dropdown label
                CustomFields::saveDropdownLabel(1, $value);
            } else if($key == 'dropdown2_label') {
                CustomFields::saveDropdownLabel(2, $value);
            } else if($key == 'dropdown3_label') {
                CustomFields::saveDropdownLabel(3, $value);
            } else if($key == 'dropdown4_label') {
                CustomFields::saveDropdownLabel(4, $value);
            } else if($key == 'dropdown5_label') {
                CustomFields::saveDropdownLabel(5, $value);
    ...
    public static function getLanguage($user_id = -1) {
        ...
        if ($user_id > 0) {
            ...
            if (empty($language)) {
                ...
                if (!file_exists(FULLCAL_DIR . '/script/lang' . strtoupper($language) . '.js')) {
                    $language = "EN";
                }
            }
        }
    ...
    public static function getAdminSetting($name, $int_user_id = -1) {
        ...
        if (!empty($arr_user)) {
            ...
            if (User::isAdmin($int_user_id)) {
                $str_query .= ' AND s.user_id = ' . $arr_user['user_id'];
            } else if (User::isUser($int_user_id)) {
                $str_query .= ' AND s.user_id = ' . $arr_user['admin_group'];
            }
            $str_query .= ' LIMIT 1 ';
        ...
        if (empty($setting)) {
            switch ($name) {
                ...
                case 'topbar_cal_logo_or_title':
                    $setting = 'only_text';
                    break;
                case 'calendar_title':
                    if (defined('CALENDAR_TITLE')) {
                        $setting = CALENDAR_TITLE;
                    } else {
                        $setting = 'Employee Work Schedule';
                    }
                    break;
                case 'language':
                    if (defined('LANGUAGE')) {
                ...
                case 'show_am_pm':
                    if (defined('SHOW_AM_PM')) {
                        $setting = SHOW_AM_PM === true ? 'on' : 'off';
                    } else {
                        $setting = 'off';
                    }
                    break;
                ...
                case 'show_dropdown_2_field':
                    $setting = 'off';
                    break;
                case 'show_dropdown_3_field':
                    $setting = 'off';
                    break;
                case 'show_dropdown_4_field':
                    $setting = 'off';
                    break;
                case 'show_dropdown_5_field':
                    $setting = 'off';
                    break;
                case 'show_url_field':
                    $setting = 'off';
                ...
                case 'show_delete_confirm_dialog':
                    if (defined('SHOW_DELETE_CONFIRM_DIALOG')) {
                        $setting = SHOW_DELETE_CONFIRM_DIALOG === true ? 'on' : 'off';
                    } else {
                        $setting = 'on';
                    }
                    break;
                case 'show_weeknumbers':
                    if (defined('SHOW_WEEKNUMBERS')) {
                        $setting = SHOW_WEEKNUMBERS === true ? 'on' : 'off';
                    } else {
                        $setting = 'on';
                    }
                    break;
                case 'truncate_title':
                    if (defined('TRUNCATE_TITLE')) {
                        $setting = TRUNCATE_TITLE === true ? 'on' : 'off';
                    } else {
                        $setting = 'off';
                    }
                    break;
                ...
                case 'show_notallowed_messages':
                    if (defined('SHOW_NOTALLOWED_MESSAGES')) {
                        $setting = SHOW_NOTALLOWED_MESSAGES === true ? 'on' : 'off';
                    } else {
                        $setting = 'off';
                    }
                    break;
                ...
                case 'send_activation_mail':
                    if (defined('SEND_ACTIVATION_MAIL')) {
                        $setting = SEND_ACTIVATION_MAIL === true ? 'on' : 'off';
                    } else {
                        $setting = 'on';
                    }
                    break;
...
//                    break;
                case 'show_public_and_private_separately':
                    if (defined('SHOW_PUBLIC_AND_PRIVATE_SEPARATELY')) {
                        $setting = SHOW_PUBLIC_AND_PRIVATE_SEPARATELY === true ? 'on' : 'off';
                    } else {
                        $setting = 'on';
                    }
                    break;
