...
if (User::isLoggedIn()) {
    ...
    $frm_submitted_sq = validate_var($arr_submit_sq);

    if(!empty($frm_submitted_sq['sq'])) {
        $obj_smarty->assign('sq', $frm_submitted_sq['sq']);
    ...
    if ($bln_superadmin) {
        ...
        $arr_users = User::getAdminUsers(true, true, $frm_submitted_sq['sq']);  // users of this admin
        $obj_smarty->assign('users', $arr_users);
    }
...
function saveProfile() {
    ...
    if (User::isAdmin() || User::isAdminUser($frm_submitted['user_id'])) {
        ...
        if (!$error || is_null($error)) {
            $frm_submitted['user_color'] = strstr($frm_submitted['user_color'], 'hsv') ? '' : $frm_submitted['user_color'];
