var Lang = {};

Lang.Fullcalendar = {};
Lang.Fullcalendar.monthNames = ['Gennaio', 'Febbraio', 'Marzo', 'Aprile', 'Maggio', 'Giugno', 'Luglio', 'Agosto', 'Settembre', 'Ottobre', 'Novembre', 'Dicembre'];
Lang.Fullcalendar.monthNamesShort = ['Gen', 'Feb', 'Mar', 'Apr', 'Mag', 'Giu', 'Lug', 'Ago', 'Set', 'Ott', 'Nov', 'Dic'];
Lang.Fullcalendar.dayNames = ['Domenica', 'Lunedì', 'Martedì', 'Mercoledì', 'Giovedì', 'Venerdì', 'Sabato'];
Lang.Fullcalendar.dayNamesShort = ['Dom', 'Lun', 'Mar', 'Mer', 'Gio', 'Ven', 'Sab'];

Lang.Fullcalendar.buttonText = {};
Lang.Fullcalendar.buttonText.today = 'Oggi';
Lang.Fullcalendar.buttonText.month = 'Mese';
Lang.Fullcalendar.buttonText.week = 'Settimana';
Lang.Fullcalendar.buttonText.day = 'Giorno';
Lang.Fullcalendar.buttonText.agendaList = 'Lista';

// DIALOG
Lang.Popup = {};
Lang.Popup.TitleAdd = 'Aggiungi evento';
Lang.Popup.TitleView = 'Visualizza evento';
Lang.Popup.TitleEdit = 'Aggiorna evento';

// DIALOG BUTTONS
Lang.Popup.closeButtonText = 'Chiudi';
Lang.Popup.saveButtonText = 'Salva';
Lang.Popup.editButtonText = 'Modifica';
Lang.Popup.cancelButtonText = 'Cancel';
Lang.Popup.deleteButtonText = 'Cancella';
Lang.Popup.saveAndRefreshButtonText = 'Salva e Aggiorna';
Lang.Popup.updateButtonText = 'Aggiorna';
Lang.Popup.addUserAndSendMailButtonText = 'Aggiungi utente e manda email';
Lang.Popup.emailText = 'Email a amministratore';
Lang.Popup.copyText = 'Copy';
Lang.Popup.copyEventText = 'Copy event';
Lang.Popup.copyToText = 'Copy to';
Lang.Popup.disconnectText = 'Disconnect';
Lang.Popup.exportToIcsText = 'ICS';

// FORM LABELS
Lang.Popup.allDayLabel = 'Tutto il giorno';
Lang.Popup.MonthLabel = 'Data';
Lang.Popup.TimeLabel = 'Ora';
Lang.Popup.StartdateLabel = 'Data inizio';
Lang.Popup.EnddateLabel = 'Data fine';
Lang.Popup.SimpleStartTimeLabel = '';
Lang.Popup.SimpleEndTimeLabel = ' - ';
Lang.Popup.EventTitle = 'Modello';
Lang.Popup.EventLocation = 'Località';
Lang.Popup.EventDescription = 'Note';
Lang.Popup.EventPhone = 'Tel.';
Lang.Popup.EventUrl = 'Url';
Lang.Popup.EventInterval = 'Intervallo';
Lang.Popup.EventRecurrence = 'Ricorrenza';
Lang.Popup.EditRecurringEvent = 'Edit recurring event';
Lang.Popup.EventTeamMember = 'Team member';

Lang.Popup.ProfileEventColor = 'Colori eventi';
Lang.Popup.EventColor = 'Colore';
Lang.Popup.EventColorUseForAllEvents = 'Utilizza per tutti i miei eventi';
Lang.Popup.DayOfMonth = 'Giorno del mese';
Lang.Popup.DayOfWeek = 'Giorno della settimana';
Lang.Popup.MonthlyOnDay = 'Giornaliero';
Lang.Popup.MonthlyOn = 'Mensile';
Lang.Popup.Starting = 'Inizio';
Lang.Popup.Daily = 'daily';
Lang.Popup.DailyOn = 'Daily on';
Lang.Popup.Every = 'ogni';
Lang.Popup.Weeks = 'settimane';
Lang.Popup.On = 'di';
Lang.Popup.WeeklyOn = 'Settimanale';
Lang.Popup.YearlyOn = 'Annuale';
Lang.Popup.Until = 'Fino a';

Lang.Popup.LabelTabMain = 'Main';
Lang.Popup.LabelTabFiles = 'Files';
Lang.Popup.LabelSalutation = 'Titulo';
Lang.Popup.LabelActive = 'Active';
Lang.Popup.LabelName = 'Nome';
Lang.Popup.LabelEmail = 'Email';
Lang.Popup.LabelUsername = 'Nome utente';
Lang.Popup.LabelLogin = 'Login';
Lang.Popup.LabelBirthdate = 'Compleanno';
Lang.Popup.LabelCountry = 'Nazione';
Lang.Popup.LabelLanguage = 'Lingua';
Lang.Popup.DefaultView = 'Visualizzazione predefinita';
Lang.Popup.LabelPassword = 'Password';
Lang.Popup.LabelPasswordAgain = 'Ripeti password';
Lang.Popup.LabelNewPassword = 'Nuova password';
Lang.Popup.LabelNewPasswordAgain = 'Ripeti nuova password';
Lang.Popup.AddUserPasswordText = 'La password sarà generata e inclusa nella mail.';
Lang.Popup.AddUserActivationMailText = 'Utente si può attivare cliccando nel link di attivazione presente nella mail.';
Lang.Popup.LabelCopyToAdmin = 'Copia a amministratore';
Lang.Popup.LabelCalendar = 'Calendario';
Lang.Popup.AssignedBy = 'Assegnato da';
Lang.Popup.Assigned = 'Assegnato';
Lang.Popup.LabelRecurring = 'Recurring';

// FORM PLACEHOLDERS
Lang.Popup.Placeholder = {};
Lang.Popup.Placeholder.LeaveBlankForNoChange = 'Lascia in bianco per nessuna modifica';
Lang.Popup.Placeholder.Firstname = 'Nome';
Lang.Popup.Placeholder.Lastname = 'Cognome';

// TIMEPICKER
Lang.Popup.TimepickerHourtext = 'Ora';
Lang.Popup.TimepickerMinutetext = 'Minuti';
Lang.Popup.TimepickercloseButtonText = 'OK';
Lang.Popup.TimepickernowButtonText = 'Adesso';

Lang.Popup.MycalendarTitle = 'Mio calendario';

// MENU
Lang.Menu = {};
Lang.Menu.TitleProfile = 'Profilo';
Lang.Menu.TitleAdduser = 'Aggiungi utente';
Lang.Menu.TitleQuickAdduser = 'Aggiungi utente velocemente';
Lang.Menu.TitleAddAdmin = 'Aggiungi amministratore';
Lang.Menu.TitleQuickAddAdmin = 'Quickly add admin';
Lang.Menu.TitleLogout = 'Esci';
Lang.Menu.TitleUsers = 'Utenti';
Lang.Menu.TitleAdmins = 'Amministratori';
Lang.Menu.TitleCalendars = 'Calendari';
Lang.Menu.TitleHourCalculation = 'Ora di calcolo';
Lang.Menu.TitleSettings = 'Impostazioni';
Lang.Menu.TitleAdminArea = 'Area amministratore';
Lang.Menu.TitleGroups = 'Groups';
Lang.Menu.TitleMyGroups = 'My groups';
Lang.Menu.TitleMyCalendars = 'My calendars';
Lang.Menu.TitleAvailability = 'Availability';
Lang.Menu.TitleAddAvailability = 'Add availability';
Lang.Menu.TitleImportUsers = 'Import users';

Lang.Button = {};
Lang.Button.addCalendar = 'Aggiungi calendario';
Lang.Button.forgottenPassword = 'Forgotten password';
Lang.Button.register = 'Register';
Lang.Button.AddDDItem = 'Add a DD-item';
Lang.Button.AddLocation = 'Add a location';
Lang.Button.SaveChanges = 'Save changes';
Lang.Button.AddGroup = 'aggiungi gruppo';
Lang.Button.Refresh = 'ricaricare';
Lang.Button.AddAvailability = 'Add availability';
Lang.Button.print = 'Print';
Lang.Button.search = 'Search';
Lang.Button.AddAnItem = 'Add an item';

// ALERTS
Lang.Alert = {};
Lang.Alert.NotAllowedToAdd = 'Non puoi aggiungere un evento.';
Lang.Alert.NotAllowedToEdit = 'Non puoi modificare questo evento.';
Lang.Alert.NotAllowedToEditGoogleEvent = 'Non puoi modificare questo evento, sarai reindirizzato a Google';
Lang.Alert.NotAllowedToAddOnThisDate = 'È esclusa questa data';
Lang.Alert.CalendarOnlyViewable = 'È possibile visualizzare solo questo calendario';

Lang.Alert.DatesNotCorrect = 'Errore nella data.';
Lang.Alert.TimesNotCorrect = 'Errore nel calendario.';
Lang.Alert.EventTitleRequired = 'Il campo Titolo è obbligatorio.';
Lang.Alert.FillInTheRequiredFields = 'Fill in the required fields';
Lang.Alert.ErrorSaving = 'Errore nel salvataggio dei dati.';
Lang.Alert.NotLoggedIn = 'La tua sessione è scaduta. Sarai reindirizzato alla pagina login.';
Lang.Alert.Error = 'Errore';
Lang.Alert.ChooseCalendarFirst = 'Devi scegliere una tipologia di calendario';
Lang.Alert.DoNotForgetToSave = 'Non dimenticarti di salvare evento!';
Lang.Alert.SettingsSavedSuccess = 'Impostazioni sono state salvate con successo';

Lang.Alert.FileTooBig = 'Il file che si sta tentando di caricare è troppo grande';
Lang.Alert.PartiallyUploaded = 'Il file che si sta tentando di upload è stato solo parzialmente caricato.';
Lang.Alert.NoFileSelected = 'Devi selezionare un file da caricare.';
Lang.Alert.ProblemWithUpload = 'Si è verificato un problema con il caricamento.';
Lang.Alert.LogInToUpload = 'Devi essere loggato per caricare i file';
Lang.Alert.FileNotAllowed = 'Questo tipo di file non è consentito';
Lang.Alert.EventAlreadyOpened = 'Questo evento è aperto da un altro, riprovare più tardi.';

Lang.Database = {};
Lang.Database.ConnectFailed = 'Connessione al database fallita';

// PROMPTS
Lang.Prompt = {};
Lang.Prompt.Disconnect = {};
Lang.Prompt.Disconnect.title = 'Disconnetti da modello';
Lang.Prompt.Disconnect.text = 'Sei sicuro di voler staccare questo elemento dal modello?';

Lang.Prompt.Update = {};
Lang.Prompt.Update.repairText = 'Ripetere?';
Lang.Prompt.Update.updateOneOrAllTitle = 'elemento aggiornato';
Lang.Prompt.Update.chooseOneOrAllText = 'Vuoi modificare questo elemento o l\'intero modello?';
Lang.Prompt.Update.thisItemBtn = 'Questo oggetto';
Lang.Prompt.Update.allItemsBtn = 'Modello intero';

Lang.Prompt.Delete = {};
Lang.Prompt.Delete.RemoveBtn = 'Rimuovere';
Lang.Prompt.Delete.CancelBtn = 'Cancella';
Lang.Prompt.Delete.ConfirmTitle = 'Rimuovere elemento';
Lang.Prompt.Delete.ConfirmText = 'Sei sicuro che vuoi rimuovere elemento?';
Lang.Prompt.Delete.chooseOneOrAllTitle = 'Rimuovi elemento';
Lang.Prompt.Delete.chooseOneOrAllText = 'Vuoi rimuovere solo questo elemento o intero modello?';
Lang.Prompt.Delete.thisItemBtn = 'Questo elemento';
Lang.Prompt.Delete.allItemsBtn = 'Modello intero';

Lang.Settings = {};
Lang.Settings.Legend = 'Impostazioni';
Lang.Settings.Infotext = 'Alcune di queste impostazioni verranno applicate solo quando qualcuno è connesso, altrimenti verranno utilizzate le impostazioni del file di configurazione.';
Lang.Settings.DefaultView = 'Visualizzazione default';
Lang.Settings.LabelWeekViewType = 'Tipo visualizzazione settimanale';
Lang.Settings.LabelDayViewType = 'Tipo visualizzazione giornaliera';
Lang.Settings.LabelLanguage = 'Lingua';
Lang.Settings.LabelOtherLanguage = 'Altre lingue';
Lang.Settings.LabelShowAmPm = 'Mostra AM/PM';
Lang.Settings.LabelShowWeeknumbers = 'Mostra N. settimana';
Lang.Settings.LabelShowNotAllowedMessages = 'Mostra messaggio "non ammesso"';
Lang.Settings.LabelTruncateTitle = 'Titolo corto';
Lang.Settings.LabelTitleLength = 'Tiolo lungo';
Lang.Settings.LabelAmountOfCharacters = 'Quantità di caratteri';
Lang.Settings.LabelEditDialog = 'Modifica finestra';
Lang.Settings.LabelColorPickerType = 'Tipo colore';
Lang.Settings.LabelTimePickerType = 'TipoTimepicker';
Lang.Settings.LabelMouseoverPopup = 'Mostra mouseoverpopup';
Lang.Settings.LabelTwoCapitals = 'Due capitali';
Lang.Settings.LabelShowDescription = 'Mostra descrizione';
Lang.Settings.LabelShowLocation = 'Mostra località';
Lang.Settings.LabelShowPhone = 'Mostra telefono';
Lang.Settings.LabelShowDeleteConfirmDialog = 'Mostra finestra conferma quando stai cancellando un elemento';
Lang.Settings.LabelHourcalculation = 'Calcolo ora';
Lang.Settings.LabelWorkdayHours = 'Ore giorno lavorativo';
Lang.Settings.LabelDefaultPeriod = 'Periodo default';
Lang.Settings.LabelWorkdayHoursInfo = 'Quantità di ore in una giornata lavorativa';
Lang.Settings.LabelDefaultPeriodInfo = 'Periodo iniziale in mesi';
Lang.Settings.LabelRegistration = 'Registrazione';
Lang.Settings.LabelRegistrationInfo = 'può essere impostato nel file config.php';
Lang.Settings.LabelSendActivationMail = 'Invia activationmail (Al momento della registrazione sul sito , l\'impostazione da config.php viene utilizzato)';

Lang.Hourcalculation = {};
Lang.Hourcalculation.legend = 'Calcolo ora';
Lang.Hourcalculation.legendOfUser = 'Calcolo ora di ';

Lang.ListView = {};
Lang.ListView.descriptionLabel = 'Info: ';
Lang.ListView.locationLabel = 'Località: ';
Lang.ListView.phoneLabel = 'Telefono: ';
Lang.ListView.urlLabel = 'Url: ';

Lang.Label = {};
Lang.Label.Yes = 'Sì';
Lang.Label.No = 'No';
Lang.Label.Color = 'a colori';
Lang.Label.BackgroundColor = 'Background color';
Lang.Label.TextColor = 'Text color';
Lang.Label.Main = 'General';
Lang.Label.ShowAll = 'Show all';
Lang.Label.UsersCanSeeNext = 'Users can see next';
Lang.Label.selectAll = 'Select all';
Lang.Label.NoItemsFound = 'No items found';

Lang.Calendar = {};
Lang.Calendar.LabelEverybody = 'Everybody';
Lang.Calendar.LabelCanView = 'can see';
Lang.Calendar.LabelGroup = 'Group ';
Lang.Calendar.LabelActivePeriod = 'periodo attivo';
Lang.Calendar.LabelAlterablePeriod = 'periodo alterabile';
Lang.Calendar.LabelActive = 'attivo';
Lang.Calendar.LabelInSpecificPeriod = 'Nel periodo specifico';
Lang.Calendar.LabelName = 'Nome';
Lang.Calendar.LabelDDItems = 'Drag & articoli goccia';
Lang.Calendar.LabelTitle = 'Titolo';
Lang.Calendar.LabelInfo = 'Info';
Lang.Calendar.LabelCanSeeDDItems = 'Può vedere DD-oggetti';
Lang.Calendar.LabelShareType = 'Condividi con';
Lang.Calendar.LabelOthers = 'Altri';
Lang.Calendar.LabelOthersCan = 'Altri può';
Lang.Calendar.LabelCanAdd = 'possibile aggiungere';
Lang.Calendar.LabelCanViewOwnAssigned = 'User can only see own assigned items' ;
Lang.Calendar.LabelCanEdit = 'può modificare';
Lang.Calendar.LabelCanDelete = 'può cancellare';
Lang.Calendar.LabelCanChangeColor = 'può cambiare colore';
Lang.Calendar.LabelDefaultCalendar = 'Calendar è aperto all\'avvio';
Lang.Calendar.LabelEventLocation = 'posizione tipo di campo';
Lang.Calendar.LabelUseColorForAllEvents = 'Usa colore di sfondo corrente per tutti gli eventi di questo calendario';
Lang.Calendar.LabelPredefinedLocations = 'luoghi predefiniti';
Lang.Calendar.LabelNotifications = 'Notifiche (solo quando un utente è loggato) ';
Lang.Calendar.LabelManually = 'manualmente';
Lang.Calendar.LabelManuallyInfo = 'Gli utenti possono spedire un evento per admin / datore di lavoro';
Lang.Calendar.LabelAutomatic = 'automatica';
Lang.Calendar.LabelAutomaticInfo = 'Un cambiamento di eventi e-mail è inviare automaticamente a admin / datore di lavoro';
Lang.Calendar.LabelCalendarAdminEmail = 'indirizzo e-mail Admin';
Lang.Calendar.LabelWhenAdminEmailEmptyInfo = 'Quando vuota: viene utilizzato MAIL_EVENT_MAILADDRESS da config.php';

Lang.Calendar.LabelDays = 'Giorni';
Lang.Calendar.LabelHours = 'Ore';
Lang.Calendar.LabelDate = 'Data';
Lang.Calendar.LabelTime = 'Orario';
Lang.Calendar.LabelCalendar = 'Calendario';
Lang.Calendar.LabelUsername = 'Nome utente';
Lang.Calendar.LabelEmail = 'E-mail';
Lang.Calendar.LabelRegistrationDate = 'Data di registrazione';
Lang.Calendar.LabelActive = 'Attivo';
Lang.Calendar.LabelCountUsers = 'conte utenti';
Lang.Calendar.LabelOwner = 'Proprietario';
Lang.Calendar.LabelOrigin = 'Origine';
Lang.Calendar.LabelCanSeeDDItems = 'Può vedere DD-articoli';
Lang.Calendar.LabelType = 'Tipo';
Lang.Calendar.LabelAdd = 'Aggiungere';
Lang.Calendar.LabelEdit = 'Modifica';
Lang.Calendar.LabelDelete = 'Cancellare';
Lang.Calendar.LabelChangeColor = 'Cambia colore';
Lang.Calendar.LabelInitialShow = 'Vista iniziale';
Lang.Calendar.LabelDDItemsCalForm = 'DD-articoli (Svuotare il campo del titolo per rimuovere la DD -item)';
Lang.Calendar.LabelDDItems = 'DD-articoli';
Lang.Calendar.LabelUsergroupDDItems = 'Gruppo di utenti DD-articoli';
Lang.Calendar.LabelAssignToUsers = 'Assegnare agli utenti';
Lang.Calendar.LabelView = 'Vista del pubblico';
Lang.Calendar.LabelAssigning = 'Assegnazione';
Lang.Calendar.LabelAddCalendar = 'Add calendar';

Lang.Calendar.LabelTableLook = 'sguardo Tabella';
Lang.Calendar.LabelShowTimeColumns = 'Mostra colonne di tempo';
Lang.Calendar.LabelShowDateOnEveryLine = 'Mostra data su ogni linea';
Lang.Calendar.LabelShowLogo = 'Mostra il tuo logo';
Lang.Calendar.LabelFontWeightBold = 'peso grassetto';
Lang.Calendar.LabelRowColorsGrayWhite = 'colori delle righe grigio / bianco';
Lang.Calendar.LabelPdfExport = 'esportazione in formato PDF';
Lang.Calendar.LabelPaginationTranslation = 'traduzione paginazione';
Lang.Calendar.LabelOf = 'di';
Lang.Calendar.LabelPage = 'pagina';
Lang.Calendar.LabelColumnNames = 'nomi delle colonne tabella';
Lang.Calendar.LabelDateColumn = 'colonna Data';
Lang.Calendar.LabelStartTimeColumn = 'Inizia colonna di tempo';
Lang.Calendar.LabelEndTimeColumn = 'colonna Ora di fine';
Lang.Calendar.LabelEventTitleColumn = 'colonna titolo';
Lang.Calendar.LabelShowCustomDropdownValues = 'Show the custom dropdown values';
Lang.Calendar.LabelUserGroupInfo = 'Se assegnando agli utenti è acceso e un elemento viene trascinato sul calendario rispetto alle ore vengono automaticamente incluse nel calcolo delle ore';
Lang.Calendar.LabelCustomDDItemInfo = 'Questi elementi non sono conteggiati nel calcolo ora perché non vi è alcun collegamento diretto con un utente. È possibile assegnare a un utente nella finestra di modifica in seguito oppure è possibile utilizzare l\'utente-gruppo DD-elementi sopra.';
Lang.Calendar.LabelSelectEmployees = 'Select employees';
        
Lang.Calendar.LabelSorting = "stessa sequenza di calendario";
Lang.Calendar.LabelShowTeamMember = 'campo squadra - membri Mostra';

Lang.Calendar.ButtonCustomList = 'elenco personalizzato';

Lang.Calendar.ShowFilter = 'Show filter';
Lang.Calendar.CalendarTitle = 'titolo del calendario';
Lang.Calendar.CalendarLogoOrTitle = 'Mostrare logo o titolo';

Lang.Options = {};
Lang.Options.Daily = 'Daily';   
Lang.Options.Weekly = 'Weekly';   
Lang.Options.Monthly = 'Monthly';   
Lang.Options.Yearly = 'Yearly';   
Lang.Options.BasicWeekView = 'Basic weekview';
Lang.Options.AgendaWeekView = 'Agenda weekview';
Lang.Options.BasicDayView = 'Basic dayview';
Lang.Options.AgendaDayView = 'Agenda dayview';
Lang.Options.czech = 'Czech';
Lang.Options.dutch = 'Dutch';
Lang.Options.english = 'English';
Lang.Options.french = 'French';
Lang.Options.german = 'German';
Lang.Options.italian = 'Italian';
Lang.Options.norwegian = 'Norwegian';
Lang.Options.polish = 'Polish';
Lang.Options.spanish = 'Spaans';
Lang.Options.Mouseover = 'Mouseover';
Lang.Options.None = 'None';    

