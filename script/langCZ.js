
var Lang = {};

Lang.Fullcalendar = {};
Lang.Fullcalendar.monthNames = ['leden', 'únor', 'březen', 'duben', 'květen', 'červen', 'červenec', 'srpen', 'září', 'říjen', 'listopad', 'prosinec'];
Lang.Fullcalendar.monthNamesShort = ['led', "úno", "bře", "dub", "můž", "čer", "čer", "srp", "zář", "říj", "lis", "pro"];
Lang.Fullcalendar.dayNames = ['nedělní', 'pondělní', 'úterní', 'středeční', 'čtvrtku', 'páteční', 'sobotní'];
Lang.Fullcalendar.dayNamesShort = ['ned', "pon", "úte", "stř", "čtv", "pát", "sob"];

Lang.Fullcalendar.buttonText = {};
Lang.Fullcalendar.buttonText.today = 'dnes';
Lang.Fullcalendar.buttonText.month = 'měsíc';
Lang.Fullcalendar.buttonText.week = 'týden';
Lang.Fullcalendar.buttonText.day = "den";
Lang.Fullcalendar.buttonText.agendaList = 'seznam';

// DIALOG
Lang.Popup = {};
Lang.Popup.TitleAdd = 'Přidat událost';
Lang.Popup.TitleView = 'Zobrazit událost';
Lang.Popup.TitleEdit = 'Upravit událost';

// DIALOG TLAČÍTKA
Lang.Popup.closeButtonText = "Zavřít";
Lang.Popup.saveButtonText = 'Save';
Lang.Popup.editButtonText = 'Edit';
Lang.Popup.cancelButtonText = 'Cancel';
Lang.Popup.deleteButtonText = 'Delete';
Lang.Popup.saveAndRefreshButtonText = 'Uložit a refresh';
Lang.Popup.updateButtonText = 'Aktualizovat';
Lang.Popup.addUserAndSendMailButtonText = 'Přidat uživatele a odesílat e-maily';
Lang.Popup.emailText = 'Email to admin';
Lang.Popup.copyText = 'Copy';
Lang.Popup.copyEventText = 'Copy event';
Lang.Popup.copyToText = 'Copy to';
Lang.Popup.disconnectText = 'Odpojit';
Lang.Popup.exportToIcsText = 'ICS';

// FORMA ŠTÍTKY
Lang.Popup.allDayLabel = 'Celý den';
Lang.Popup.MonthLabel = 'Datum';
Lang.Popup.TimeLabel = 'Čas';
Lang.Popup.StartdateLabel = 'StartDate';
Lang.Popup.EnddateLabel = 'EndDate';
Lang.Popup.SimpleStartTimeLabel = 'Z';
Lang.Popup.SimpleEndTimeLabel = 'Do';
Lang.Popup.EventTitle = "hlava";
Lang.Popup.EventLocation = 'Umístění';
Lang.Popup.EventDescription = 'Popis';
Lang.Popup.EventPhone = 'Phone';
Lang.Popup.EventUrl = 'url';
Lang.Popup.EventInterval = 'Interval';
Lang.Popup.EventRecurrence = 'Opakování';
Lang.Popup.EditRecurringEvent = 'Edit recurring event';
Lang.Popup.EventTeamMember = 'Team member';

Lang.Popup.ProfileEventColor = 'Akce color';
Lang.Popup.EventColor = 'Color';
Lang.Popup.EventColorUseForAllEvents = 'Používá se pro al mé události';
Lang.Popup.DayOfMonth = "Den v měsíci";
Lang.Popup.DayOfWeek = "Den v týdnu";
Lang.Popup.MonthlyOnDay = 'Měsíční den';
Lang.Popup.MonthlyOn = 'Měsíční na';
Lang.Popup.Starting = "výchozí";
Lang.Popup.Daily = 'daily';
Lang.Popup.DailyOn = 'Daily on';
Lang.Popup.Every = 'každé';
Lang.Popup.Weeks = 'týdny';
Lang.Popup.On = 'v';
Lang.Popup.WeeklyOn = 'Weekly na';
Lang.Popup.YearlyOn = 'Roční na';
Lang.Popup.Until = 'Do';

Lang.Popup.LabelTabMain = 'Hlavní';
Lang.Popup.LabelTabFiles = 'Soubory';
Lang.Popup.LabelSalutation = 'Titul';
Lang.Popup.LabelActive = 'Aktivní';
Lang.Popup.LabelName = 'name';
Lang.Popup.LabelEmail = 'Email';
Lang.Popup.LabelUsername = 'Uživatelské jméno';
Lang.Popup.LabelLogin = 'Přihlášení';
Lang.Popup.LabelBirthdate = 'Datum narození';
Lang.Popup.LabelCountry = "Země";
Lang.Popup.LabelLanguage = 'jazyk';
Lang.Popup.DefaultView = 'Výchozí zobrazení';
Lang.Popup.LabelPassword = 'Heslo';
Lang.Popup.LabelPasswordAgain = 'Heslo znovu';
Lang.Popup.LabelNewPassword = 'nové heslo';
Lang.Popup.LabelNewPasswordAgain = 'znovu nové heslo';
Lang.Popup.AddUserPasswordText = 'Heslo bude vygenerováno a jsou zahrnuty v e-mailu.';
Lang.Popup.AddUserActivationMailText = 'Uživatel může aktivovat s aktivačním odkazem zahrnuty v e-mailu.';
Lang.Popup.LabelCopyToAdmin = 'Kopírovat admin';
Lang.Popup.LabelCalendar = 'Kalendář';
Lang.Popup.AssignedBy = 'Přidělené';
Lang.Popup.Assigned = 'Účelově';
Lang.Popup.LabelRecurring = 'Recurring';

// FORMA zástupné symboly
Lang.Popup.Placeholder = {};
Lang.Popup.Placeholder.LeaveBlankForNoChange = 'Nechte prázdné pro změnu';
Lang.Popup.Placeholder.Firstname = 'Jméno';
Lang.Popup.Placeholder.Lastname = 'příjmení';

// TIMEPICKER
Lang.Popup.TimepickerHourtext = 'Hour';
Lang.Popup.TimepickerMinutetext = 'Minuta';
Lang.Popup.TimepickercloseButtonText = 'OK';
Lang.Popup.TimepickernowButtonText = 'Now';

Lang.Popup.MycalendarTitle = "Můj kalendář";

// MENU
Lang.Menu = {};
Lang.Menu.TitleProfile = 'Profil';
Lang.Menu.TitleAdduser = 'Přidat uživatele';
Lang.Menu.TitleAddAdmin = 'Přidat admin';
Lang.Menu.TitleQuickAdduser = 'Rychle přidat uživatele';
Lang.Menu.TitleQuickAddAdmin = 'Rychle přidat admin';
Lang.Menu.TitleLogout = 'Odhlásit';
Lang.Menu.TitleUsers = 'Users';
Lang.Menu.TitleAdmins = 'Admins';
Lang.Menu.TitleCalendars = 'Calendars';
Lang.Menu.TitleHourCalculation = 'Hour Calculation';
Lang.Menu.TitleSettings = "Nastavení";
Lang.Menu.TitleAdminArea = 'administrativní oblasti';
Lang.Menu.TitleGroups = 'Groups';
Lang.Menu.TitleMyGroups = 'My groups';
Lang.Menu.TitleMyCalendars = 'My calendars';
Lang.Menu.TitleAvailability = 'Availability';
Lang.Menu.TitleAddAvailability = 'Add availability';
Lang.Menu.TitleImportUsers = 'Import users';

Lang.Button = {};
Lang.Button.addCalendar = 'Přidat kalendáře';
Lang.Button.forgottenPassword = 'Zapomenuté heslo';
Lang.Button.register = 'Registrace';
Lang.Button.AddDDItem = 'Add a DD-item';
Lang.Button.AddLocation = 'Add a location';
Lang.Button.SaveChanges = 'Save changes';
Lang.Button.AddGroup = 'přidat skupinu';
Lang.Button.Refresh = 'Obnovit';
Lang.Button.AddAvailability = 'Add availability';
Lang.Button.print = 'vytisknout';
Lang.Button.search = 'vyhledávání';
Lang.Button.AddAnItem = 'Add an item';


// ZÁZNAMY
Lang.Alert = {};
Lang.Alert.NotAllowedToAdd = "Nemůžeš přidat událost.";
Lang.Alert.NotAllowedToEdit = 'Nemůžete měnit tuto událost.';
Lang.Alert.NotAllowedToEditGoogleEvent = 'Nemůžete měnit tuto událost, budete přesměrováni na Google';
Lang.Alert.NotAllowedToAddOnThisDate = 'This date is excluded';
Lang.Alert.CalendarOnlyViewable = 'Můžete zobrazit pouze tento kalendář';

Lang.Alert.DatesNotCorrect = 'Chyba v datech.';
Lang.Alert.TimesNotCorrect = 'Chyba v listinách';
Lang.Alert.EventTitleRequired = 'je zapotřebí v názvu';
Lang.Alert.FillInTheRequiredFields = 'Fill in the required fields';
Lang.Alert.ErrorSaving = 'Chyba při ukládání.';
Lang.Alert.NotLoggedIn = 'Vaše přihlášení vypršela. Budete přesměrováni na přihlašovací stránku. ';
Lang.Alert.ChooseCalendarFirst = 'Choose one calendar first';
Lang.Alert.DoNotForgetToSave = 'Do not forget to save the event !';
Lang.Alert.Error = 'Chyba';
Lang.Alert.SettingsSavedSuccess = 'Nastavení úspěšně uloženo, aktualizujte vidět změny';

Lang.Alert.FileTooBig = 'Soubor, který se pokoušíte nahrát, je příliš velký';
Lang.Alert.PartiallyUploaded = 'Soubor, který se pokoušíte nahrát byl nahrán jen částečně.';
Lang.Alert.NoFileSelected = 'Musíte vybrat soubor k nahrání.';
Lang.Alert.ProblemWithUpload = "Došlo k potížím s upload.";
Lang.Alert.LogInToUpload = "Musíte být přihlášeni právo vkládat";
Lang.Alert.FileNotAllowed = "Tento typ souboru není povolen";
Lang.Alert.EventAlreadyOpened = 'Tato událost se otevře někdo jiný, zkuste to znovu později.';

Lang.Database = {};
Lang.Database.ConnectFailed = 'Připojení k databázi se nezdařilo';

// Vyzve
Lang.Prompt = {};
Lang.Prompt.Disconnect = {};
Lang.Prompt.Disconnect.title = 'Odpojení od vzoru';
Lang.Prompt.Disconnect.text = 'Jste si jisti, že chcete odpojit tuto položku ze vzoru?';

Lang.Prompt.Update = {};
Lang.Prompt.Update.repairText = 'Oprava opakování vzoru?';
Lang.Prompt.Update.updateOneOrAllTitle = 'Edit item(s)';
Lang.Prompt.Update.chooseOneOrAllText = 'Do you want to edit this item or the whole pattern?';
Lang.Prompt.Update.thisItemBtn = 'This item';
Lang.Prompt.Update.allItemsBtn = 'Whole pattern';

Lang.Prompt.Delete = {};
Lang.Prompt.Delete.RemoveBtn = 'Odstranit';
Lang.Prompt.Delete.CancelBtn = 'Zrušit';
Lang.Prompt.Delete.ConfirmTitle = 'odebrat';
Lang.Prompt.Delete.ConfirmText = 'Jste si jisti, že chcete odstranit tuto položku?';
Lang.Prompt.Delete.chooseOneOrAllTitle = 'Odstranit položku (y)';
Lang.Prompt.Delete.chooseOneOrAllText = "Přejete si odstranit pouze tuto položku nebo celou vzor?";
Lang.Prompt.Delete.thisItemBtn = 'Tato položka';
Lang.Prompt.Delete.allItemsBtn = 'Celý vzor';

Lang.Settings = {};
Lang.Settings.Legend = "Nastavení";
Lang.Settings.Infotext = 'Některá z těchto nastavení bude použita pouze tehdy, když je někdo přihlášen, jinak bude použito nastavení z config.php.';
Lang.Settings.DefaultView = 'Výchozí zobrazení';
Lang.Settings.LabelWeekViewType = 'Weekview typ';
Lang.Settings.LabelDayViewType = 'typ denním pohledem';
Lang.Settings.LabelLanguage = 'jazyk';
Lang.Settings.LabelOtherLanguage = 'Jiný jazyk';
Lang.Settings.LabelShowAmPm = 'Zobrazit AM / PM';
Lang.Settings.LabelShowWeeknumbers = 'Ukázat weeknumbers';
Lang.Settings.LabelShowNotAllowedMessages = 'Zobrazit "není povoleno" zprávy';
Lang.Settings.LabelMouseoverPopup = 'zobrazit myší pop-up';
Lang.Settings.LabelTruncateTitle = 'Zkraťte název';
Lang.Settings.LabelTitleLength = 'délka hlava';
Lang.Settings.LabelAmountOfCharacters = 'Počet znaků';
Lang.Settings.LabelEditDialog = 'Edit dialog';
Lang.Settings.LabelColorPickerType = 'typ ColorPicker';
Lang.Settings.LabelTimePickerType = 'Timepicker typ';
Lang.Settings.LabelTwoCapitals = "Dva kapitály";
Lang.Settings.LabelShowDescription = 'Ukázat popis';
Lang.Settings.LabelShowLocation = 'Show location';
Lang.Settings.LabelShowPhone = 'Zobrazit telefon';
Lang.Settings.LabelShowDeleteConfirmDialog = 'Zobrazit Potvrdit dialog při mazání položky';
Lang.Settings.LabelHourcalculation = 'výpočet Hour';
Lang.Settings.LabelWorkdayHours = 'pracovní den hodiny';
Lang.Settings.LabelDefaultPeriod = 'Výchozí období';
Lang.Settings.LabelWorkdayHoursInfo = 'Počet hodin v pracovní den';
Lang.Settings.LabelDefaultPeriodInfo = "Počáteční doba v měsících,";
Lang.Settings.LabelRegistration = 'Registrace';
Lang.Settings.LabelRegistrationInfo = 'lze nastavit v config.php';
Lang.Settings.LabelSendActivationMail = 'Odeslat activationmail (Při registraci na webové stránce , nastavení z config.php je používán)';


Lang.Hourcalculation = {};
Lang.Hourcalculation.legend = 'Hour calculation';
Lang.Hourcalculation.legendOfUser = 'Hour calculation of ';

Lang.ListView = {};
Lang.ListView.descriptionLabel = 'Upozornění:';
Lang.ListView.locationLabel = 'Umístění:';
Lang.ListView.phoneLabel = 'Telefon:';
Lang.ListView.urlLabel = 'url';

Lang.Label = {};
Lang.Label.Yes = 'Ano';
Lang.Label.No = 'Ne';
Lang.Label.Color = 'Color';
Lang.Label.BackgroundColor = 'Background color';
Lang.Label.TextColor = 'Text color';
Lang.Label.Main = 'General';
Lang.Label.ShowAll = 'Show all';
Lang.Label.UsersCanSeeNext = 'Users can see next';
Lang.Label.selectAll = ' Select all';
Lang.Label.NoItemsFound = 'nalezeny žádné položky';

Lang.Calendar = {};
Lang.Calendar.LabelEverybody = 'Everybody';
Lang.Calendar.LabelCanView = 'can see';
Lang.Calendar.LabelGroup = 'Group ';
Lang.Calendar.LabelActivePeriod = "Aktivní období";
Lang.Calendar.LabelAlterablePeriod = 'Modifikovatelné období';
Lang.Calendar.LabelActive = 'Aktivní';
Lang.Calendar.LabelInSpecificPeriod = "V určitém časovém období";
Lang.Calendar.LabelName = 'name';
Lang.Calendar.LabelDDItems = 'Drag & Drop položky';
Lang.Calendar.LabelTitle = "hlava";
Lang.Calendar.LabelInfo = "Info";
Lang.Calendar.LabelCanSeeDDItems = 'Je vidět, DD-položky';
Lang.Calendar.LabelShareType = 'Podělte se s';
Lang.Calendar.LabelOthers = "Ostatní";
Lang.Calendar.LabelOthersCan = 'Ostatní umi';
Lang.Calendar.LabelCanAdd = 'můžete přidat';
Lang.Calendar.LabelCanViewOwnAssigned = 'User can only see own assigned items' ;
Lang.Calendar.LabelCanEdit = 'lze upravit';
Lang.Calendar.LabelCanDelete = 'můžete smazat';
Lang.Calendar.LabelCanChangeColor = 'může změnit barvu';
Lang.Calendar.LabelDefaultCalendar = 'Kalendář je otevřena na startu';
Lang.Calendar.LabelEventLocation = 'Umístění typ pole';
Lang.Calendar.LabelUseColorForAllEvents = 'Použít aktuální barva pozadí pro všechny události v tomto kalendáři';
Lang.Calendar.LabelPredefinedLocations = 'Předdefinované lokality';
Lang.Calendar.LabelNotifications = "Upozornění (pouze když je uživatel přihlášen)";
Lang.Calendar.LabelManually = 'Ručně';
Lang.Calendar.LabelManuallyInfo = 'Uživatelé si mohou poštou událost do admin / zaměstnavateli';
Lang.Calendar.LabelAutomatic = 'Automatic';
Lang.Calendar.LabelAutomaticInfo = 'An změny email událost je automaticky odeslat na admin / zaměstnavateli';
Lang.Calendar.LabelCalendarAdminEmail = 'správce e-mailová adresa';
Lang.Calendar.LabelWhenAdminEmailEmptyInfo = 'Po vyprázdnění: Užívá se MAIL_EVENT_MAILADDRESS z config.php';

Lang.Calendar.LabelDays = 'dny';
Lang.Calendar.LabelHours = 'hodiny';
Lang.Calendar.LabelDate = 'Datum';
Lang.Calendar.LabelTime = 'Čas';
Lang.Calendar.LabelCalendar = 'Kalendář';
Lang.Calendar.LabelUsername = 'Uživatelské jméno';
Lang.Calendar.LabelEmail = 'E-mail';
Lang.Calendar.LabelRegistrationDate = 'Datum registrace';
Lang.Calendar.LabelActive = 'Aktivní';
Lang.Calendar.LabelCountUsers = 'hrabě uživatelů';
Lang.Calendar.LabelOwner = 'Majitel';
Lang.Calendar.LabelOrigin = 'Původ';
Lang.Calendar.LabelCanSeeDDItems = 'Je vidět, DD-položek';
Lang.Calendar.LabelType = 'Typ';
Lang.Calendar.LabelAdd = 'Přidat';
Lang.Calendar.LabelEdit = 'Edit';
Lang.Calendar.LabelDelete = 'Vymazat';
Lang.Calendar.LabelChangeColor = 'Změnit barvu';
Lang.Calendar.LabelInitialShow = 'Počáteční výstava';
Lang.Calendar.LabelDDItemsCalForm = 'DD-položek (Vyprázdněte pole název odebrat DD- položky)';
Lang.Calendar.LabelDDItems = 'DD-položek';
Lang.Calendar.LabelUsergroupDDItems = 'Uživatel DD-položek';
Lang.Calendar.LabelAssignToUsers = 'Přiřadit uživatele';
Lang.Calendar.LabelView = 'Veřejné pohled';
Lang.Calendar.LabelAssigning = 'Přiřazení';
Lang.Calendar.LabelAddCalendar = 'Add calendar';

Lang.Calendar.LabelTableLook = 'Tabulka look';
Lang.Calendar.LabelShowTimeColumns = 'Zobrazit čas sloupce';
Lang.Calendar.LabelShowDateOnEveryLine = 'Zobrazit datum na každém řádku';
Lang.Calendar.LabelShowLogo = 'Ukázat své logo';
Lang.Calendar.LabelFontWeightBold = 'hmotnost písma bold';
Lang.Calendar.LabelRowColorsGrayWhite = 'Row barvy šedá / bílá';
Lang.Calendar.LabelPdfExport = 'exportovat do PDF';
Lang.Calendar.LabelPaginationTranslation = 'Stránkování překlad';
Lang.Calendar.LabelOf = 'z';
Lang.Calendar.LabelPage = 'strana';
Lang.Calendar.LabelColumnNames = 'Názvy sloupec tabulky';
Lang.Calendar.LabelDateColumn = 'sloupce Datum';
Lang.Calendar.LabelStartTimeColumn = 'Počáteční čas sloupec';
Lang.Calendar.LabelEndTimeColumn = 'Čas konce sloupec';
Lang.Calendar.LabelEventTitleColumn = 'Název sloupce';
Lang.Calendar.LabelShowCustomDropdownValues = 'Show the custom dropdown values';
Lang.Calendar.LabelUserGroupInfo = 'Pokud uživatel přiřazení zapnutý a položka přesunuta do kalendáře, než hodiny jsou automaticky zahrnuty do výpočtu hodin';
Lang.Calendar.LabelCustomDDItemInfo = 'Tyto položky se nezapočítává do výpočtu hodinu, protože neexistuje žádná přímá souvislost s uživatelem. Můžete přiřadit k uživateli v dialogu pro úpravy později, nebo můžete použít uživatelské skupiny DD-položky seznamu.';
Lang.Calendar.LabelSelectEmployees = 'Select employees';
        
Lang.Calendar.LabelSorting = "stejně jako kalendářním sledu";
Lang.Calendar.LabelShowTeamMember = 'Zobrazit pole člena týmu';

Lang.Calendar.ButtonCustomList = 'vlastní seznam';

Lang.Calendar.ShowFilter = 'Show filter';
Lang.Calendar.CalendarTitle = 'Titulní kalendář';
Lang.Calendar.CalendarLogoOrTitle = 'Zobrazí logo nebo název';

Lang.Options = {};
Lang.Options.Daily = 'Daily';   
Lang.Options.Weekly = 'Weekly';   
Lang.Options.Monthly = 'Monthly';   
Lang.Options.Yearly = 'Yearly';   
Lang.Options.BasicWeekView = 'Basic weekview';
Lang.Options.AgendaWeekView = 'Agenda weekview';
Lang.Options.BasicDayView = 'Basic dayview';
Lang.Options.AgendaDayView = 'Agenda dayview';
Lang.Options.czech = 'Czech';
Lang.Options.dutch = 'Dutch';
Lang.Options.english = 'English';
Lang.Options.french = 'French';
Lang.Options.german = 'German';
Lang.Options.italian = 'Italian';
Lang.Options.norwegian = 'Norwegian';
Lang.Options.polish = 'Polish';
Lang.Options.spanish = 'Spaans';
Lang.Options.Mouseover = 'Mouseover';
Lang.Options.None = 'None';    
