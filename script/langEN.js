var Lang = {};

Lang.Fullcalendar = {};
Lang.Fullcalendar.monthNames = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
Lang.Fullcalendar.monthNamesShort = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
Lang.Fullcalendar.dayNames = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
Lang.Fullcalendar.dayNamesShort = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];

Lang.Fullcalendar.buttonText = {};
Lang.Fullcalendar.buttonText.today = 'today';
Lang.Fullcalendar.buttonText.month = 'month';
Lang.Fullcalendar.buttonText.week = 'week';
Lang.Fullcalendar.buttonText.day = 'day';
Lang.Fullcalendar.buttonText.agendaList = 'list';


// DIALOG
Lang.Popup = {};
Lang.Popup.TitleAdd = 'Add an event';
Lang.Popup.TitleView = 'View an event';
Lang.Popup.TitleEdit = 'Edit an event';

// DIALOG BUTTONS
Lang.Popup.closeButtonText = 'Close';
Lang.Popup.saveButtonText = 'Save';
Lang.Popup.editButtonText = 'Edit';
Lang.Popup.cancelButtonText = 'Cancel';
Lang.Popup.deleteButtonText = 'Delete';
Lang.Popup.saveAndRefreshButtonText = 'Save & refresh';
Lang.Popup.updateButtonText = 'Update';
Lang.Popup.addUserAndSendMailButtonText = 'Add user and send email';
Lang.Popup.emailText = 'Email to admin';
Lang.Popup.copyText = 'Copy';
Lang.Popup.copyEventText = 'Copy event';
Lang.Popup.copyToText = 'Copy to';
Lang.Popup.disconnectText = 'Disconnect';
Lang.Popup.exportToIcsText = 'ICS';

// FORM LABELS
Lang.Popup.allDayLabel = 'All day';
Lang.Popup.MonthLabel = 'Date';
Lang.Popup.TimeLabel = 'Time';
Lang.Popup.StartdateLabel = 'Startdate';
Lang.Popup.EnddateLabel = 'Enddate';
Lang.Popup.SimpleStartTimeLabel = '';
Lang.Popup.SimpleEndTimeLabel = ' - ';
Lang.Popup.EventTitle = 'Title';
Lang.Popup.EventLocation = 'Location';
Lang.Popup.EventDescription = 'Description';
Lang.Popup.EventPhone = 'Phone';
Lang.Popup.EventUrl = 'Url';
Lang.Popup.EventInterval = 'Interval';
Lang.Popup.EventRecurrence = 'Recurrence';
Lang.Popup.EditRecurringEvent = 'Edit recurring event';
Lang.Popup.EventTeamMember = 'Team member';

Lang.Popup.ProfileEventColor = 'Events color';
Lang.Popup.EventColor = 'Color';
Lang.Popup.EventColorUseForAllEvents = 'Use for al my events';
Lang.Popup.DayOfMonth = 'Day of month';
Lang.Popup.DayOfWeek = 'Day of week';
Lang.Popup.MonthlyOnDay = 'Monthly on day';
Lang.Popup.MonthlyOn = 'Monthly on';
Lang.Popup.Starting = 'starting';
Lang.Popup.Daily = 'daily';
Lang.Popup.DailyOn = 'Daily on';
Lang.Popup.Every = 'every';
Lang.Popup.Weeks = 'weeks';
Lang.Popup.On = 'on';
Lang.Popup.WeeklyOn = 'Weekly on';
Lang.Popup.YearlyOn = 'Yearly on';
Lang.Popup.Until = 'Until';

Lang.Popup.LabelTabMain = 'Main';
Lang.Popup.LabelTabFiles = 'Files';
Lang.Popup.LabelSalutation = 'Salutation';
Lang.Popup.LabelActive = 'Active';
Lang.Popup.LabelName = 'Name';
Lang.Popup.LabelEmail = 'Email';
Lang.Popup.LabelUsername = 'Username';
Lang.Popup.LabelLogin = 'Login';
Lang.Popup.LabelBirthdate = 'Birthdate';
Lang.Popup.LabelCountry = 'Country';
Lang.Popup.LabelLanguage = 'Language';
Lang.Popup.DefaultView = 'Default view';
Lang.Popup.LabelPassword = 'Password';
Lang.Popup.LabelPasswordAgain = 'Password again';
Lang.Popup.LabelNewPassword = 'New Password';
Lang.Popup.LabelNewPasswordAgain = 'New password again';
Lang.Popup.AddUserPasswordText = 'A password will be generated and included in the email.';
Lang.Popup.AddUserActivationMailText = 'The user can activate with the activation link included in the email.';
Lang.Popup.LabelCopyToAdmin = 'Copy to admin';
Lang.Popup.LabelCalendar = 'Calendar';
Lang.Popup.AssignedBy = 'Assigned by';
Lang.Popup.Assigned = 'Assigned';
Lang.Popup.LabelRecurring = 'Recurring';

// FORM PLACEHOLDERS
Lang.Popup.Placeholder = {};
Lang.Popup.Placeholder.LeaveBlankForNoChange = 'Leave blank for no change';
Lang.Popup.Placeholder.Firstname = 'Firstname';
Lang.Popup.Placeholder.Lastname = 'Lastname';

// TIMEPICKER
Lang.Popup.TimepickerHourtext = 'Hour';
Lang.Popup.TimepickerMinutetext = 'Minute';
Lang.Popup.TimepickercloseButtonText = 'OK';
Lang.Popup.TimepickernowButtonText = 'Now';

Lang.Popup.MycalendarTitle = 'My calendar';

// MENU
Lang.Menu = {};
Lang.Menu.TitleProfile = 'Profile';
Lang.Menu.TitleAdduser = 'Add user';
Lang.Menu.TitleQuickAdduser = 'Quickly add user';
Lang.Menu.TitleAddAdmin = 'Add admin';
Lang.Menu.TitleQuickAddAdmin = 'Quickly add admin';
Lang.Menu.TitleLogout = 'Log out';
Lang.Menu.TitleUsers = 'Users';
Lang.Menu.TitleAdmins = 'Admins';
Lang.Menu.TitleCalendars = 'Calendars';
Lang.Menu.TitleHourCalculation = 'Hour Calculation';
Lang.Menu.TitleSettings = 'Settings';
Lang.Menu.TitleAdminArea = 'Admin area';
Lang.Menu.TitleGroups = 'Groups';
Lang.Menu.TitleMyGroups = 'My groups';
Lang.Menu.TitleMyCalendars = 'My calendars';
Lang.Menu.TitleAvailability = 'Availability';
Lang.Menu.TitleAddAvailability = 'Add availability';
Lang.Menu.TitleImportUsers = 'Import users';

Lang.Button = {};
Lang.Button.addCalendar = 'Add calendar';
Lang.Button.forgottenPassword = 'Forgotten password';
Lang.Button.register = 'Register';
Lang.Button.AddDDItem = 'Add a DD-item';
Lang.Button.AddLocation = 'Add a location';
Lang.Button.SaveChanges = 'Save changes';
Lang.Button.AddGroup = 'Add group';
Lang.Button.Refresh = 'Refresh';
Lang.Button.AddAvailability = 'Add availability';
Lang.Button.print = 'Print';
Lang.Button.search = 'Search';
Lang.Button.AddAnItem = 'Add an item';

// ALERTS
Lang.Alert = {};
Lang.Alert.NotAllowedToAdd = 'You cannot add an event.';
Lang.Alert.NotAllowedToEdit = 'You cannot modify this event.';
Lang.Alert.NotAllowedToEditGoogleEvent = 'You cannot modify this event, you will be redirected to Google';
Lang.Alert.NotAllowedToAddOnThisDate = 'This date is excluded';
Lang.Alert.CalendarOnlyViewable = 'You can only view this calendar';

Lang.Alert.DatesNotCorrect = 'Error in the dates.';
Lang.Alert.TimesNotCorrect = 'Error in the schedules.';
Lang.Alert.EventTitleRequired = 'Title is required.';
Lang.Alert.FillInTheRequiredFields = 'Fill in the required fields';
Lang.Alert.ErrorSaving = 'Error while saving.';
Lang.Alert.NotLoggedIn = 'Your login is expired. You will be redirected to the login page.';
Lang.Alert.Error = 'Error';
Lang.Alert.ChooseCalendarFirst = 'Choose one calendar first';
Lang.Alert.DoNotForgetToSave = 'Do not forget to save the event !';
Lang.Alert.SettingsSavedSuccess = 'Settings saved successfully';

Lang.Alert.FileTooBig = 'The file you are trying to upload is too big';
Lang.Alert.PartiallyUploaded = 'The file you are trying to upload was only partially uploaded.';
Lang.Alert.NoFileSelected = 'You must select a file for upload.';
Lang.Alert.ProblemWithUpload = 'There was a problem with your upload.';
Lang.Alert.LogInToUpload = 'You have to be logged in to upload files';
Lang.Alert.FileNotAllowed = 'This type of file is not allowed';
Lang.Alert.EventAlreadyOpened = 'This event is opened by someone else, try again later.';

Lang.Database = {};
Lang.Database.ConnectFailed = 'Database connection failed';

// PROMPTS
Lang.Prompt = {};
Lang.Prompt.Disconnect = {};
Lang.Prompt.Disconnect.title = 'Disconnect from pattern';
Lang.Prompt.Disconnect.text = 'Are you sure you want to disconnect this item from the pattern?';

Lang.Prompt.Update = {};
Lang.Prompt.Update.repairText = 'Repair repeating pattern?';
Lang.Prompt.Update.updateOneOrAllTitle = 'Edit item(s)';
Lang.Prompt.Update.chooseOneOrAllText = 'Do you want to edit this item or the whole pattern?';
Lang.Prompt.Update.thisItemBtn = 'This item';
Lang.Prompt.Update.allItemsBtn = 'Whole pattern';

Lang.Prompt.Delete = {};
Lang.Prompt.Delete.RemoveBtn = 'Remove';
Lang.Prompt.Delete.CancelBtn = 'Cancel';
Lang.Prompt.Delete.ConfirmTitle = 'Remove item';
Lang.Prompt.Delete.ConfirmText = 'Are you sure you want to remove this item?';
Lang.Prompt.Delete.chooseOneOrAllTitle = 'Remove item(s)';
Lang.Prompt.Delete.chooseOneOrAllText = 'Do you want to remove only this item or the whole pattern?';
Lang.Prompt.Delete.thisItemBtn = 'This item';
Lang.Prompt.Delete.allItemsBtn = 'Whole pattern';

Lang.Settings = {};
Lang.Settings.Legend = 'Settings';
Lang.Settings.Infotext = 'Some of these settings will only be applied when someone is logged in, otherwise the settings from config.php will be used.';
Lang.Settings.DefaultView = 'Default view';
Lang.Settings.LabelWeekViewType = 'Weekview type';
Lang.Settings.LabelDayViewType = 'Dayview type';
Lang.Settings.LabelLanguage = 'Language';
Lang.Settings.LabelOtherLanguage = 'Other language';
Lang.Settings.LabelShowAmPm = 'Show AM/PM';
Lang.Settings.LabelShowWeeknumbers = 'Show weeknumbers';
Lang.Settings.LabelShowNotAllowedMessages = 'Show "Not allowed" messages';
Lang.Settings.LabelTruncateTitle = 'Shorten title';
Lang.Settings.LabelTitleLength = 'Title length';
Lang.Settings.LabelAmountOfCharacters = 'Amount of characters';
Lang.Settings.LabelEditDialog = 'Edit-dialog';
Lang.Settings.LabelColorPickerType = 'Colorpicker type';
Lang.Settings.LabelTimePickerType = 'Timepicker type';
Lang.Settings.LabelMouseoverPopup = 'Show mouseoverpopup';
Lang.Settings.LabelTwoCapitals = 'Two capitals';
Lang.Settings.LabelShowDescription = 'Show description field';
Lang.Settings.LabelShowLocation = 'Show location field';
Lang.Settings.LabelShowPhone = 'Show phone field';
Lang.Settings.LabelShowDeleteConfirmDialog = 'Show confirm dialog when deleting an item';
Lang.Settings.LabelHourcalculation = 'Hour calculation';
Lang.Settings.LabelWorkdayHours = 'Workday hours';
Lang.Settings.LabelDefaultPeriod = 'Default period';
Lang.Settings.LabelWorkdayHoursInfo = 'Amount of hours in a workday';
Lang.Settings.LabelDefaultPeriodInfo = 'Initial period in months';
Lang.Settings.LabelRegistration = 'Registration';
Lang.Settings.LabelRegistrationInfo = 'can be set in config.php';
Lang.Settings.LabelSendActivationMail = 'Send activationmail (When registering on the website, the setting from config.php is used)';

Lang.Hourcalculation = {};
Lang.Hourcalculation.legend = 'Hour calculation';
Lang.Hourcalculation.legendOfUser = 'Hour calculation of ';

Lang.ListView = {};
Lang.ListView.descriptionLabel = 'Info: ';
Lang.ListView.locationLabel = 'Location: ';
Lang.ListView.phoneLabel = 'Phone: ';
Lang.ListView.urlLabel = 'Url: ';

Lang.Label = {};
Lang.Label.Yes = 'Yes';
Lang.Label.No = 'No';
Lang.Label.Color = 'Color';
Lang.Label.BackgroundColor = 'Background color';
Lang.Label.TextColor = 'Text color';
Lang.Label.Main = 'General';
Lang.Label.ShowAll = 'Show all';
Lang.Label.UsersCanSeeNext = 'Users can see next';
Lang.Label.selectAll = 'Select all';
Lang.Label.NoItemsFound = 'No items found';

Lang.Calendar = {};
Lang.Calendar.LabelEverybody = 'Everybody';
Lang.Calendar.LabelCanView = 'can see';
Lang.Calendar.LabelGroup = 'Group ';
Lang.Calendar.LabelActivePeriod = 'Active period';
Lang.Calendar.LabelAlterablePeriod = 'Alterable period';
Lang.Calendar.LabelActive = 'Active';
Lang.Calendar.LabelInSpecificPeriod = 'In specific period';
Lang.Calendar.LabelName = 'Name';
Lang.Calendar.LabelDDItems = 'Drag & Drop items, empty a title to delete the DD-item';
Lang.Calendar.LabelTitle = 'Title';
Lang.Calendar.LabelInfo = 'Info';
Lang.Calendar.LabelCanSeeDDItems = 'Can see DD-items';
Lang.Calendar.LabelShareType = 'Share with';
Lang.Calendar.LabelOthers = 'Others ';
Lang.Calendar.LabelOthersCan = 'Others can';
Lang.Calendar.LabelCanAdd = 'can add';
Lang.Calendar.LabelCanViewOwnAssigned = 'User can only see own assigned items' ;
Lang.Calendar.LabelCanEdit = 'can edit';
Lang.Calendar.LabelCanDelete = 'can delete';
Lang.Calendar.LabelCanChangeColor = 'can change color';
Lang.Calendar.LabelDefaultCalendar = 'Calendar is open on start';
Lang.Calendar.LabelEventLocation = 'Location field type';
Lang.Calendar.LabelUseColorForAllEvents = 'Use current background color for all the events of this calendar';
Lang.Calendar.LabelPredefinedLocations = 'Predefined locations';
Lang.Calendar.LabelNotifications = 'Notifications (only when a user is logged in)';
Lang.Calendar.LabelManually = 'Manually';
Lang.Calendar.LabelManuallyInfo = 'Users can mail an event to admin/employer';
Lang.Calendar.LabelAutomatic = 'Automatic';
Lang.Calendar.LabelAutomaticInfo = 'An email with event changes is automatically send to admin/employer';
Lang.Calendar.LabelCalendarAdminEmail = 'Admin email address';
Lang.Calendar.LabelWhenAdminEmailEmptyInfo = 'When empty: MAIL_EVENT_MAILADDRESS from config.php is used';

Lang.Calendar.LabelDays = 'Days';
Lang.Calendar.LabelHours = 'Hours';
Lang.Calendar.LabelDate = 'Date';
Lang.Calendar.LabelTime = 'Time';
Lang.Calendar.LabelCalendar = 'Calendar';
Lang.Calendar.LabelUsername = 'Username';
Lang.Calendar.LabelEmail = 'Email';
Lang.Calendar.LabelRegistrationDate = 'Registration date';
Lang.Calendar.LabelActive = 'Active';
Lang.Calendar.LabelCountUsers = 'Count users';
Lang.Calendar.LabelOwner = 'Owner';
Lang.Calendar.LabelOrigin = 'Origin';
Lang.Calendar.LabelCanSeeDDItems = 'Can see DD-items';
Lang.Calendar.LabelType = 'Type';
Lang.Calendar.LabelAdd = 'Add';
Lang.Calendar.LabelEdit = 'Edit';
Lang.Calendar.LabelDelete = 'Delete';
Lang.Calendar.LabelChangeColor = 'Change color';
Lang.Calendar.LabelInitialShow = 'Initial show';
Lang.Calendar.LabelDDItemsCalForm = 'Custom DD-items (Empty the title field to remove the DD-item)';
Lang.Calendar.LabelDDItems = 'Custom DD-items';
Lang.Calendar.LabelUsergroupDDItems = 'User-group DD-items';
Lang.Calendar.LabelAssignToUsers = 'Assign to users';
Lang.Calendar.LabelView = 'Public view';
Lang.Calendar.LabelAssigning = 'Assigning';
Lang.Calendar.LabelAddCalendar = 'Add calendar';
Lang.Calendar.LabelTableLook = 'Table look';
Lang.Calendar.LabelShowTimeColumns = 'Show time columns';
Lang.Calendar.LabelShowDateOnEveryLine = 'Show date on every line';
Lang.Calendar.LabelShowLogo = 'Show your logo';
Lang.Calendar.LabelFontWeightBold = 'Fontweight bold';
Lang.Calendar.LabelRowColorsGrayWhite = 'Row colors gray/white';
Lang.Calendar.LabelPdfExport = 'PDF export';
Lang.Calendar.LabelPaginationTranslation = 'Pagination translation';
Lang.Calendar.LabelOf = 'of';
Lang.Calendar.LabelPage = 'page';
Lang.Calendar.LabelColumnNames = 'Table column names';
Lang.Calendar.LabelDateColumn = 'Date column';
Lang.Calendar.LabelStartTimeColumn = 'Start time column';
Lang.Calendar.LabelEndTimeColumn = 'End time column';
Lang.Calendar.LabelEventTitleColumn = 'Title column';
Lang.Calendar.LabelShowCustomDropdownValues = 'Show the custom dropdown values';
Lang.Calendar.LabelUserGroupInfo = 'When \'assign to users\' is on and dragging onto the calendar, the hours are counted in the hour-calculation';
Lang.Calendar.LabelCustomDDItemInfo = 'These items are not counted in the hour calculation because there is no direct link with a user. You can assign to a user in the edit dialog afterwards or you can use the user-group DD-items above.';
Lang.Calendar.LabelSelectEmployees = 'Select employees';

Lang.Calendar.LabelSorting = "Sorting same as calendar sequence";
Lang.Calendar.LabelShowTeamMember = 'Show team-member field';

Lang.Calendar.ButtonCustomList = 'custom list';

Lang.Calendar.ShowFilter = 'Show filter';
Lang.Calendar.CalendarTitle = 'Calendar title';
Lang.Calendar.CalendarLogoOrTitle = 'Show Logo / title';

Lang.Options = {};
Lang.Options.Daily = 'Daily';   
Lang.Options.Weekly = 'Weekly';   
Lang.Options.Monthly = 'Monthly';   
Lang.Options.Yearly = 'Yearly';   
Lang.Options.BasicWeekView = 'Basic weekview';
Lang.Options.AgendaWeekView = 'Agenda weekview';
Lang.Options.BasicDayView = 'Basic dayview';
Lang.Options.AgendaDayView = 'Agenda dayview';
Lang.Options.czech = 'Czech';
Lang.Options.dutch = 'Dutch';
Lang.Options.english = 'English';
Lang.Options.french = 'French';
Lang.Options.german = 'German';
Lang.Options.italian = 'Italian';
Lang.Options.norwegian = 'Norwegian';
Lang.Options.polish = 'Polish';
Lang.Options.spanish = 'Spaans';
Lang.Options.Mouseover = 'Mouseover';
Lang.Options.None = 'None';    