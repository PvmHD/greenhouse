...
$arr_global_settings = Settings::getSettings();

$bln_found = false;
foreach ($current_languages as $code => $lang) {
    if (strtoupper($arr_global_settings['language']) == $code) {
        $bln_found = true;
        $obj_smarty->assign('language', $code);
    }
}
if (!$bln_found) {
    if (!file_exists(FULLCAL_DIR . '/script/lang' . strtoupper($arr_global_settings['language']) . '.js')) {
...
if (User::isLoggedIn()) {
    ...
    $obj_smarty->assign('is_user', User::isUser());
...
if (isset($_GET['action'])) {
    switch ($_GET['action']) {
        ...
        case 'agenda':
            showPeriodListView();
            break;
        case 'get_changed_events':
            getChangedEvents();
            break;
        case 'get_day_amount_available_employees':
            getDayAmountAvailableEmployees();
            break;
        case 'get_amount_available_employees':
            getAmountAvailableEmployees();
            break;
        case 'getfcd':
            echo FULLCAL_DIR;
            break;
    ...
    $frm_submitted_get = validate_var($arr_submit);
    ...
    if(!empty($frm_submitted_get['cid'])) {
        $str_initial_calendars = ltrim($frm_submitted_get['cid'],',');
        $str_initial_calendars = rtrim($frm_submitted_get['cid'],',');
        
        $arr_selected_calendars = explode(',',$str_initial_calendars);
        $arr_selected_calendars = array_unique($arr_selected_calendars);
        
        $str_initial_calendars = implode(',',$arr_selected_calendars);
    ...
    if(empty($str_initial_calendars) && isset($arr_calendars[0])) {
    ...
    $bln_initial_cal_show_dropdown3 = false;
    $bln_initial_cal_show_dropdown4 = false;
    $bln_initial_cal_show_dropdown5 = false;
    ...
    foreach ($arr_calendars as &$def) {
        // when coming from custom listview
        if(!empty($frm_submitted_get['cid'])) {
            if(in_array($def['calendar_id'], $arr_selected_calendars)) {
                $def['initial_show'] = true;
            }
        }
...
//        }
    ...
    $obj_smarty->assign('dropdown2_label', CustomFields::getDropdownLabel(2)); 
    $obj_smarty->assign('dropdown1_label', CustomFields::getDropdownLabel(1)); 
    $obj_smarty->assign('dropdown3_label', CustomFields::getDropdownLabel(3)); 
    $obj_smarty->assign('dropdown4_label', CustomFields::getDropdownLabel(4)); 
    $obj_smarty->assign('dropdown5_label', CustomFields::getDropdownLabel(5)); 
   ...
   // $obj_smarty->assign('current_dropdown_options', true); 
    ...
    $obj_smarty->assign('dropdown1', CustomFields::getCustomDropdown(1));
    $obj_smarty->assign('dropdown2', CustomFields::getCustomDropdown(2));
    $obj_smarty->assign('dropdown3', CustomFields::getCustomDropdown(3));
    $obj_smarty->assign('dropdown4', CustomFields::getCustomDropdown(4));
    $obj_smarty->assign('dropdown5', CustomFields::getCustomDropdown(5));
    ...
    if (isset($arr_calendars[0])) {
        ...
        $obj_smarty->assign('default_calendar_textcolor', $arr_calendars[0]['calendar_textcolor']);
        ...
        $obj_smarty->assign('default_calendar_textcolor', '#FFFFFF');
    ...
    if (!empty($str_initial_calendars)) {
        ...
        if (!strstr($str_initial_calendars, ',')) {
            ...
            $obj_smarty->assign('default_calendar_textcolor', $arr_cal['calendar_textcolor']);
        } else {
        ...
        if (!empty($first_default_calendar)) {
            ...
            $obj_smarty->assign('default_calendar_textcolor', $first_default_calendar['calendar_textcolor']);
        }
    }
    ...
    if (!empty($arr_cal)) {
        ...
        $arr_users_grouped = User::getUsersByGroup(true, true, $arr_cal, true);
   //print_r($arr_users);     
        $obj_smarty->assign('users', $arr_users);
        $obj_smarty->assign('grouped_users', $arr_users_grouped);
...
$arr_submit = array(
   ...
   // array('cl', 'int', false, 0),
    array('cid', 'string', false, ''),
    array('fa', 'int', false, 0),   // from agenda, when defaultview is 'customList'
);
...
//    if($frm_submitted_initial['cl'] === 1) {
//        $obj_smarty->assign('close_after_update', true);
//
//    }
...
if (User::isLoggedIn()) {
    ...
    if(User::isAdmin()) {
        Utils::addLog('refresh', $arr_user['user_id']);
    }
    ...
    if(CUSTOM_LISTVIEW_IS_DEFAULT_VIEW_FOR_USERS_AND_NOT_LOGGEDIN_VIEWERS && (($arr_settings['default_view'] == 'customList' || User::isUser($arr_user['user_id'])) && $frm_submitted_initial['fa'] == 0)) {
        header('location: ' . FULLCAL_URL . '/?action=agenda'); 
        exit;
    }
...
//		unset($_SESSION['calendar-uid']);
    ...
    $obj_smarty->assign('is_user', false);
    $obj_smarty->assign('is_super_admin', false);
    $obj_smarty->assign('is_owner', false);

    $arr_settings = Settings::getSettings();

    if(($arr_settings['default_view'] == 'customList' || CUSTOM_LISTVIEW_IS_DEFAULT_VIEW_FOR_USERS_AND_NOT_LOGGEDIN_VIEWERS === true) && $frm_submitted_initial['fa'] == 0) {
        header('location: ' . FULLCAL_URL . '/?action=agenda'); 
        exit;
    }
...
function search() {
    ...
    $arr_submit = array(
        ...
        array('custom_period_start', 'string', false, ''),
        array('custom_period_end', 'string', false, ''),
    );
    ...
    if (!empty($frm_submitted)) {
        ...
        if (!empty($arr_calendars)) {
            foreach ($arr_calendars as $calendar) {
                $str_query = 'SELECT e.*, re.*, event_id as id, concat_ws(" ",date_start,time_start) as start,concat_ws(" ",date_end,time_end) as end FROM events e' .
                        ' LEFT JOIN `repeating_events` re ON(re.rep_event_id = e.repeating_event_id) ' ;
                
                $str_query .= ' WHERE (`title` LIKE  "%' . $frm_submitted['sq'] . '%" OR `description` LIKE  "%' . $frm_submitted['sq'] . '%" OR `location` LIKE  "%' . $frm_submitted['sq'] . '%")' ;
                
                $str_query .= ($user_id > 0 && $calendar['share_type'] != "public" && ALLOW_ACCESS_BY !== 'free' && !Calendar::UserInGroup($calendar, $user_id) ? ' AND user_id = ' . $user_id : '');

                if(!empty($frm_submitted['custom_period_start']) && !empty($frm_submitted['custom_period_end'])) {
                    $str_query .= ' AND ("'.$frm_submitted['custom_period_start'].'" BETWEEN `date_start` AND `date_end` OR  "'.$frm_submitted['custom_period_end'].'" BETWEEN `date_start` AND `date_end` )';
                }
        ...
        foreach ($arr_events as $event) {
            ...
            $event['team_member'] = '';
            if (isset($event['team_member_id']) && !empty($event['team_member_id'])) {
                $event['team_member'] = Utils::getTeamMemberNames($event['team_member_id']);
               
            }
            
            $event['show_dropdown_1_field'] = CalendarOption::getOption('add_custom_dropdown1_to_title', $event['calendar_id'], false); 
            $event['show_dropdown_2_field'] = CalendarOption::getOption('add_custom_dropdown2_to_title', $event['calendar_id'], false); 
            $event['show_dropdown_3_field'] = CalendarOption::getOption('add_custom_dropdown3_to_title', $event['calendar_id'], false); 
            $event['show_dropdown_4_field'] = CalendarOption::getOption('add_custom_dropdown4_to_title', $event['calendar_id'], false); 
            $event['show_dropdown_5_field'] = CalendarOption::getOption('add_custom_dropdown5_to_title', $event['calendar_id'], false); 
            
            $event['dropdown1'] = '';
            if($event['show_dropdown_1_field']) {
                if(isset($event['dropdown1_option_id']) && $event['dropdown1_option_id'] > 0) {
                    if (isset($_SESSION['ews_custom_field_options']) && isset($_SESSION['ews_custom_field_options'][$event['dropdown1_option_id']]) && !empty($_SESSION['ews_custom_field_options'][$event['dropdown1_option_id']])) {
                        $arr_option = $_SESSION['ews_custom_field_options'][$event['dropdown1_option_id']];
                    } else {
                        $arr_option = CustomFields::getCustomDropdownOption($arr_line['dropdown1_option_id']);
                        $_SESSION['ews_custom_field_options'][$event['dropdown1_option_id']] = $arr_option;
                    }
                    $event['dropdown1'] = $arr_option['text'];
                    $event['dropdown1_color'] = $arr_option['color'];
            ...
            if($event['show_dropdown_2_field']) {
                if(isset($event['dropdown2_option_id']) && $event['dropdown2_option_id'] > 0) {
                    if (isset($_SESSION['ews_custom_field_options']) && isset($_SESSION['ews_custom_field_options'][$event['dropdown2_option_id']]) && !empty($_SESSION['ews_custom_field_options'][$event['dropdown2_option_id']])) {
                        $arr_option = $_SESSION['ews_custom_field_options'][$event['dropdown2_option_id']];
                    } else {
                        $arr_option = CustomFields::getCustomDropdownOption($event['dropdown2_option_id']);
                        $_SESSION['ews_custom_field_options'][$event['dropdown2_option_id']] = $arr_option;
                    }
                    $event['dropdown2'] = $arr_option['text'];
                    $event['dropdown2_color'] = $arr_option['color'];
                }
            }
            
            $event['dropdown3'] = '';
            if($event['show_dropdown_3_field']) {
                if(isset($event['dropdown3_option_id']) && $event['dropdown3_option_id'] > 0) {
                    if (isset($_SESSION['ews_custom_field_options']) && isset($_SESSION['ews_custom_field_options'][$event['dropdown3_option_id']]) && !empty($_SESSION['ews_custom_field_options'][$event['dropdown3_option_id']])) {
                        $arr_option = $_SESSION['ews_custom_field_options'][$event['dropdown3_option_id']];
                    } else {
                        $arr_option = CustomFields::getCustomDropdownOption($event['dropdown3_option_id']);
                        $_SESSION['ews_custom_field_options'][$event['dropdown3_option_id']] = $arr_option;
                    }
                    $event['dropdown3'] = $arr_option['text'];
                    $event['dropdown3_color'] = $arr_option['color'];
                }
            }
            
            $event['dropdown4'] = '';
            if($event['show_dropdown_4_field']) {
                if(isset($event['dropdown4_option_id']) && $event['dropdown4_option_id'] > 0) {
                    if (isset($_SESSION['ews_custom_field_options']) && isset($_SESSION['ews_custom_field_options'][$event['dropdown4_option_id']]) && !empty($_SESSION['ews_custom_field_options'][$event['dropdown4_option_id']])) {
                        $arr_option = $_SESSION['ews_custom_field_options'][$event['dropdown4_option_id']];
                    } else {
                        $arr_option = CustomFields::getCustomDropdownOption($event['dropdown4_option_id']);
                        $_SESSION['ews_custom_field_options'][$event['dropdown4_option_id']] = $arr_option;
                    }
                    $event['dropdown4'] = $arr_option['text'];
                    $event['dropdown4_color'] = $arr_option['color'];
                }
            }
            
            $event['dropdown5'] = '';
            if($event['show_dropdown_5_field']) {
                if(isset($event['dropdown5_option_id']) && $event['dropdown5_option_id'] > 0) {
                    if (isset($_SESSION['ews_custom_field_options']) && isset($_SESSION['ews_custom_field_options'][$event['dropdown5_option_id']]) && !empty($_SESSION['ews_custom_field_options'][$event['dropdown5_option_id']])) {
                        $arr_option = $_SESSION['ews_custom_field_options'][$event['dropdown5_option_id']];
                    } else {
                        $arr_option = CustomFields::getCustomDropdownOption($event['dropdown5_option_id']);
                        $_SESSION['ews_custom_field_options'][$event['dropdown5_option_id']] = $arr_option;
                    }
                    $event['dropdown5'] = $arr_option['text'];
                    $event['dropdown5_color'] = $arr_option['color'];
                }
            }
...
function exportOneToIcs() {
    ...
    Utils::exportToIcs($frm_submitted['event_id']);
}
...
function createCsv() {
    ...
    //Array ( [start] => 2016-12-01 [end] => 2017-01-01 [cal_id] => 1 [df] => d-m-y [m] => d [cv] => ) 
    ...
    foreach ($arr_calendar_ids as $cal_id) {
        if (isset($_SESSION['ews_calendars']) && isset($_SESSION['ews_calendars'][$cal_id]) && !empty($_SESSION['ews_calendars'][$cal_id])) {
            ...
            if(isset($_SESSION['ews_calendars'][$cal_id]['calendar_textcolor'])) {
                $calendar_textcolor = $_SESSION['ews_calendars'][$cal_id]['calendar_textcolor'];
            }
            ...
            if(isset($arr_calendar['calendar_textcolor'])) {
                $calendar_textcolor = $arr_calendar['calendar_textcolor'];
            }
    ...
    $dropdown1_label = CustomFields::getDropdownLabel(1);
    $dropdown2_label = CustomFields::getDropdownLabel(2);
    $dropdown3_label = CustomFields::getDropdownLabel(3);
    $dropdown4_label = CustomFields::getDropdownLabel(4);
    $dropdown5_label = CustomFields::getDropdownLabel(5);
    ...
    $arr_simplicated_events = array();
//    foreach($arr_events as &$event) {print_r($event);
//       
//      //  $arr_columns[] = $event
//      //  $arr_simplicated_events[] = $event;
//       
//        
//    }exit;
    
    $arr_simplicated_events[] = array("title", "calendar",  "teammember", "location", "startdate", "enddate", "starttime", "endtime",$dropdown1_label,$dropdown2_label,$dropdown3_label,$dropdown4_label,$dropdown5_label, "description", "url");
            
    foreach($arr_events as &$event) {//print_r($event);
        unset($event['users']);
        unset($event['rep_event']);
        //if(!is_array($event)) {
        $arr_row = array();
        $arr_row[] = $event['title'];
        $arr_row[] = $event['calendar_name'];
        $arr_row[] = str_replace(array('&#60;','>'),'',strip_tags($event['team_member']));
        $arr_row[] = $event['location'];
        $arr_row[] = $event['date_start'];
        $arr_row[] = $event['date_end'];
        $arr_row[] = $event['time_start'];
        $arr_row[] = $event['time_end'];
        
        if($event['dropdown1_option_id'] > 0) {
            if (isset($_SESSION['ews_custom_field_options']) && isset($_SESSION['ews_custom_field_options'][$event['dropdown1_option_id']]) && !empty($_SESSION['ews_custom_field_options'][$event['dropdown1_option_id']])) {
                $arr_option = $_SESSION['ews_custom_field_options'][$event['dropdown1_option_id']];
            } else {
                $arr_option = CustomFields::getCustomDropdownOption($arr_line['dropdown1_option_id']);
            ...
            $arr_row[] = $arr_option['text'];
        } else {
            $arr_row[] = '';
        }
        if($event['dropdown2_option_id'] > 0) {
            if (isset($_SESSION['ews_custom_field_options']) && isset($_SESSION['ews_custom_field_options'][$event['dropdown2_option_id']]) && !empty($_SESSION['ews_custom_field_options'][$event['dropdown2_option_id']])) {
                $arr_option = $_SESSION['ews_custom_field_options'][$event['dropdown2_option_id']];
            } else {
        ...
        if($event['dropdown3_option_id'] > 0) {
            if (isset($_SESSION['ews_custom_field_options']) && isset($_SESSION['ews_custom_field_options'][$event['dropdown3_option_id']]) && !empty($_SESSION['ews_custom_field_options'][$event['dropdown3_option_id']])) {
                $arr_option = $_SESSION['ews_custom_field_options'][$event['dropdown3_option_id']];
            } else {
        ...
        if($event['dropdown4_option_id'] > 0) {
            if (isset($_SESSION['ews_custom_field_options']) && isset($_SESSION['ews_custom_field_options'][$event['dropdown4_option_id']]) && !empty($_SESSION['ews_custom_field_options'][$event['dropdown4_option_id']])) {
                $arr_option = $_SESSION['ews_custom_field_options'][$event['dropdown4_option_id']];
            } else {
        ...
        if($event['dropdown5_option_id'] > 0) {
            if (isset($_SESSION['ews_custom_field_options']) && isset($_SESSION['ews_custom_field_options'][$event['dropdown5_option_id']]) && !empty($_SESSION['ews_custom_field_options'][$event['dropdown5_option_id']])) {
                $arr_option = $_SESSION['ews_custom_field_options'][$event['dropdown5_option_id']];
            } else {
        ...
        $arr_row[] = $event['description'];
        $arr_row[] = $event['myurl'];
        
        
            $arr_simplicated_events[] = $arr_row;
        //}
    ...
    outputCSV($arr_simplicated_events);
}
        ...
        foreach ($arr_events as $key => $row) {
            ...
            if($show_custom_dropdown_values) {
                ...
                if((isset($row['dropdown1']) && !empty($row['dropdown1'])) || (isset($row['dropdown2']) && !empty($row['dropdown2']))
                        || (isset($row['dropdown3']) && !empty($row['dropdown3'])) || (isset($row['dropdown4']) && !empty($row['dropdown4']))
                        || (isset($row['dropdown5']) && !empty($row['dropdown5']))) {
                    $custom_dropdowns_added = true;
                    $title .= ' (';
                }
                if(isset($row['dropdown1']) && !empty($row['dropdown1'])) {
                    $title_with_cdd .= (!empty($title_with_cdd) ? ', ' : '') . $row['dropdown1'];
                }
                if(isset($row['dropdown2']) && !empty($row['dropdown2'])) {
                    $title_with_cdd .= (!empty($title_with_cdd) ? ', ' : '') . $row['dropdown2'];
                }
                if(isset($row['dropdown3']) && !empty($row['dropdown3'])) {
                    $title_with_cdd .= (!empty($title_with_cdd) ? ', ' : '') . $row['dropdown3'];
                }
                if(isset($row['dropdown4']) && !empty($row['dropdown4'])) {
                    $title_with_cdd .= (!empty($title_with_cdd) ? ', ' : '') . $row['dropdown4'];
                }
                if(isset($row['dropdown5']) && !empty($row['dropdown5'])) {
                    $title_with_cdd .= (!empty($title_with_cdd) ? ', ' : '') . $row['dropdown5'];
                }

                $title .= $title_with_cdd;
...
//                $total_string_width = $pdf->GetStringWidth($title);
//                $number_of_lines = $total_string_width / 60;
//                $number_of_lines = ceil( $number_of_lines );  // Round it up.
//                
//                $line_height = 6;                             // Whatever your line height is.
//                $height_of_cell = $number_of_lines * $line_height; 
//                $height_of_cell = ceil( $height_of_cell );    // Round it up.
//echo '<pre>'.$height_of_cell.'</pre>';
              //  if($number_of_lines > 1) {
                    //$pdf->SetY($pdf->GetY()-6);
              //  }
                
                if ($show_times) {
                    $height_of_cell = $pdf->GetMultiCellHeight(70, 6, $title);  
                } else {
                    $height_of_cell = $pdf->GetMultiCellHeight(125, 6, $title);  
                }
                
      
//echo '<pre>'.$height_of_cell.'</pre>'; 
                $pdf->Cell($w[0], $height_of_cell, date($dateformat, strtotime($row['date_start'])), 'LR', 0, 'L', $fill);
                
               

                if($show_calendarname_each_line) {
                    $pdf->Cell($w[0], $height_of_cell, $_SESSION['ews_calendars'][$row['calendar_id']]['name'], 'L', 0, 'L', $fill);
                }
         // echo '<pre>'.strlen($title).'</pre>';      
                if ($show_times) {
                    if ($row['allDay']) {
                        $pdf->Cell($w[1] + $w[2], $height_of_cell, 'allDay', 'L', 0, 'L', $fill);
                    } else {
                        $pdf->Cell($w[1], $height_of_cell, $start_time . ' - ', 'L', 0, '', $fill);
                        $pdf->Cell($w[2], $height_of_cell, $end_time, 0, 0, '', $fill);
                    }
                    
                    $pdf->MultiCell(70, 6, $title, 'LR', 'L', $fill);
                } else {
                    
                    $pdf->MultiCell(125, 6, $title, 'LR', 'L', $fill);
                }
            } else {
                if ($show_times) {
                    $height_of_cell = $pdf->GetMultiCellHeight(70, 6, $title);  
        ...
        foreach ($arr_sorted_events as $date => $arr_event) {
            ...
            foreach ($arr_event as $key => $row) {
                ...
                if ($table_look) {
                   if ($show_times) {
                        $height_of_cell = $pdf->GetMultiCellHeight(95, 6, $title);  
                    } else {
                        $height_of_cell = $pdf->GetMultiCellHeight(145, 6, $title);  
                    }

                    if($show_calendarname_each_line) {
                        if(!isset($_SESSION['ews_calendars']) || !isset($_SESSION['ews_calendars'][$row['calendar_id']])) {
                            $arr_calendar = Calendar::getCalendar($row['calendar_id']);
                            $_SESSION['ews_calendars'][$row['calendar_id']] = $arr_calendar;
                        }
                        $pdf->Cell(40, $height_of_cell, $_SESSION['ews_calendars'][$row['calendar_id']]['name'], 'L', 0, 'L', $fill);
                    } else {
                        $pdf->Cell(40, $height_of_cell, "", 'LR', 0, 'L', $fill);
                    }
                        // $pdf->Cell($w[0],6,date('d-m-Y', strtotime($row['date_start'])),'LR',0,'L',$fill);

                    if ($show_times) {
                        if ($row['allDay']) {
                            $pdf->Cell($w[1] + $w[2], $height_of_cell, 'allDay', 'LR', 0, '', $fill);
                        } else {
                            $pdf->Cell($w[1], $height_of_cell, $start_time . ' - ', 'L', 0, '', $fill);
                            $pdf->Cell($w[2], $height_of_cell, $end_time, '', 0, '', $fill);
                        }
                        $pdf->MultiCell(95, 6, $title, 'LR', 'L', $fill);
                    } else {
                        $pdf->MultiCell(145, 6, $title, 'LR', 'L', $fill);
                    }
                } else {
                    $pdf->SetFont('Arial', $bold ? 'B' : '', 12);
    ...
    $pdf->setXY($page_nr_x, $page_nr_y);
    $pdf->SetTextColor(0);
    $pdf->Multicell($totalwidth, 6, $lang_page . ' ' . $pdf->PageNo() . ' ' . $lang_of . ' {nb}', 0);
...
function getChangedEvents() {
    Utils::getChangedEvents();
    //echo json_encode(array('success' => true, 'title'=>'hihi'));
    exit;
}

function getDayAmountAvailableEmployees() {
    $arr_amount_employees = array();
    ...
    $arr_submit = array(
        array('user_id', 'int', true, ''),
        array('visStart', 'string', true, ''),
        array('visEnd', 'string', false, ''),
    ...
    if(defined('AVAILABLE_EMPLOYEES_GROUP')) {
        $int_group_id = AVAILABLE_EMPLOYEES_GROUP;
        $arr_group_users = Group::getUserIds($int_group_id);
        $cnt_users_in_group = count(Group::getUserIds($int_group_id));
    } else {
        $cnt_users_in_group = 0;
    }
    
     
    if($cnt_users_in_group == 0) {
        $arr_amount_employees = array(-1,-1,-1,-1,-1);
    } else {
        // check how many employees are scheduled
        $date = $frm_submitted['visStart'];

        if(!defined('SHOW_WEEKENDS')) {
            define('SHOW_WEEKENDS', true);
        }
        if(SHOW_WEEKENDS) {
            // get date of first visible day of the week
            $date = date('Y-m-d', strtotime('- '.date('w', strtotime($date)) . ' DAY', strtotime($date)));
            
        } else {
           // echo date('w', strtotime($date));
            $int_minus_days = date('w', strtotime($date)) - 1;
            
            // get date of first visible day of the week
            $date = date('Y-m-d', strtotime('- '.$int_minus_days . ' DAY', strtotime($date)));
        }
        
   // echo $date;
        $cnt_assigned_users = Events::getCountAssignedUsers($date, $int_group_id, $arr_group_users);
        $arr_amount_employees[] = $cnt_users_in_group - $cnt_assigned_users;
     ...
     echo json_encode(array('success' => true, 'amount_employees' => $arr_amount_employees));
    exit;
}

function getAmountAvailableEmployees() {
    $arr_amount_employees = array();
    ...
    if(defined('AVAILABLE_EMPLOYEES_GROUP')) {
        $int_group_id = AVAILABLE_EMPLOYEES_GROUP;
        $arr_group_users = Group::getUserIds($int_group_id);
        $cnt_users_in_group = count($arr_group_users);
    } else {
        $cnt_users_in_group = 0;
    }
...
function fixTeammembers() {
        ...
        $str_query = 'SELECT * FROM events e  WHERE team_member_id != "" AND team_member_id IS NOT NULL AND team_member_id NOT LIKE %_%';
        ...
        if($obj_result !== false) {
            while ($arr_line = mysqli_fetch_array($obj_result, MYSQLI_ASSOC)) {
                ...
                $arr_teammember = array();
                foreach($arr_teammembers as $teammember) {
                    $arr_teammember[] = '5_' . $teammember;
                }
                
                $str_update_query = 'UPDATE `events` SET team_member_id = "' . implode(',', $arr_teammember) . '" WHERE event_id = '.$arr_line['event_id'];

                mysqli_query($obj_db, $str_update_query);
